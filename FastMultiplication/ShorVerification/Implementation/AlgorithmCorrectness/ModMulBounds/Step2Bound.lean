...
/-- The active work and grown data-carry registers are disjoint. -/
private lemma alg1_step2_work_dataCarry_disjoint
    {η : ℝ} (cfg : ModMulConfig η) :
    Shor.Disjoint cfg.env.work.active (cfg.env.data.grow 1).active := by
...
/-- The symmetric form of work/data-carry disjointness. -/
private lemma alg1_step2_dataCarry_work_disjoint
    {η : ℝ} (cfg : ModMulConfig η) :
    Shor.Disjoint (cfg.env.data.grow 1).active cfg.env.work.active :=
  Shor.Disjoint.symm (alg1_step2_work_dataCarry_disjoint cfg)

/-- Two fresh data reserve bits in particular provide one fresh reserve bit. -/
private lemma alg1_step2_data_freshFor_one_of_two
    {Basis : Type v} [RegEncoding Basis]
    {η : ℝ} (cfg : ModMulConfig η) (b : Basis)
    (hfresh : cfg.env.data.FreshFor 2 b) :
    cfg.env.data.FreshFor 1 b :=
  ExtReg.freshFor_one_of_two
    cfg.env.data b cfg.env.circuit_workspace.1 hfresh

/-- Taking two list elements and dropping the first equals taking one from the tail. -/
private lemma alg1_step2_take_two_tail_eq_tail_take_one
    {α : Type*} (xs : List α) :
...
/--
After growing the data register by one bit, the remaining next reserve bit is
fresh whenever the original register had two fresh reserve bits.
-/
private lemma alg1_step2_dataCarry_freshFor_one_of_two
    {Basis : Type v} [RegEncoding Basis]
    {η : ℝ} (cfg : ModMulConfig η) (b : Basis)
    (hfresh : cfg.env.data.FreshFor 2 b) :
    (cfg.env.data.grow 1).FreshFor 1 b := by
...
/-- Writing an owned-disjoint active register preserves reserve freshness. -/
private lemma alg1_step2_freshFor_write_active_of_ownedDisjoint
    {Basis : Type v} [RegEncoding Basis]
    (target writer : ExtReg)
    (n value : ℕ)
    (b : Basis)
    (hdisj : ExtReg.OwnedDisjoint target writer)
    (hfresh : target.FreshFor n b) :
    target.FreshFor n
      (RegEncoding.writeNat writer.active value b) :=
  ExtReg.freshFor_write_active_of_ownedDisjoint
    target writer n value b hdisj hfresh

/--
The Step-2 workspace remains clean after writing both the work label and the
grown data-carry value.
-/
private lemma alg1_step2_workspace_clean_after_dataCarry_work_write
    (qs : QSemantics)
  ...
  change
    cfg.env.work.FreshFor 1
        (RegEncoding.writeNat
          (cfg.env.data.grow 1).active
          y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) ∧ (cfg.env.data.grow 1).FreshFor 1
        (RegEncoding.writeNat
          (cfg.env.data.grow 1).active
          y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b))
  constructor
  · exact alg1_step2_freshFor_write_active_of_ownedDisjoint
      cfg.env.work
      (cfg.env.data.grow 1)
      1
      y.1
      ...
      (ExtReg.freshFor_write_active cfg.env.work 1 t.1 b hb.2.2.2.1)
  · exact ExtReg.freshFor_write_active
      (cfg.env.data.grow 1)
      1
      y.1
      (RegEncoding.writeNat cfg.env.work.active t.1 b)
      (alg1_step2_freshFor_write_active_of_ownedDisjoint
        (cfg.env.data.grow 1)
        cfg.env.work
        1
        t.1
        ...
        (alg1_step2_dataCarry_freshFor_one_of_two cfg b hb.2.1))
...
lemma alg1_step2_source_label_injective_on_good
  ...
  have htu_val : t.1 = u.1 := by
    calc
      t.1 = RegEncoding.toNat cfg.env.work.active
          (RegEncoding.writeNat cfg.env.work.active t.1 b) := by
            symm
            exact
...
/--
Expand the Step-2 trace error as the coherent sum of the individual branch
errors over retained basis/work labels.
-/
lemma alg1_step2_trace_error_eq_good_branch_sum
    ...
    qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg) tr.goodStep1
      - tr.afterStep2Ref = ∑ i ∈ Sgood,
      α i •
        (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
          (qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) - qs.ket
            (RegEncoding.writeNat
              ((cfg.env.data.grow 1).active)
        ...
        α i •
          qs.ket
            (RegEncoding.writeNat
              ...
              (alg1Step2Value cfg i.1)
              (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) := by
    simp [Sgood, α, Alg1Trace.afterStep2Ref, Finset.sum_sigma, Finset.smul_sum, smul_smul]

  calc
    qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg) tr.goodStep1
        - tr.afterStep2Ref = qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
        (∑ i ∈ Sgood,
          α i • qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) - ∑ i ∈ Sgood,
        α i •
          qs.ket
            (RegEncoding.writeNat
              ((cfg.env.data.grow 1).active)
...
lemma alg1_step2_good_coeff_energy_eq_norm_sq
    ...
    let α : (Σ _b : qs.Basis, Fin (ASize cfg.env.work.active)) → ℂ :=
      fun i => tr.inputCoeff i.1 * tr.phaseCoeff i.1 i.2
    ∑ i ∈ Sgood, ‖α i‖ ^ 2 = ‖tr.goodStep1‖ ^ 2 := by
  classical
  dsimp only
  ...
  calc
    ∑ i ∈ Sgood, ‖α i‖ ^ 2 = ∑ i ∈ Sgood, ‖α i • qs.ket (labelGood i)‖ ^ 2 := by
        apply Finset.sum_congr rfl
        intro i hi
...
/--
For a valid unit input, the total squared coefficient energy of the retained
Step-1 packet is at most one.
-/
...
/--
A retained work label approximates its target residue with discrepancy strictly
less than the requested precision `η`.
-/
...
lemma alg1_step2_input_xext_value
    ...
    RegEncoding.toNat (cfg.env.data.grow 1).active
      (RegEncoding.writeNat cfg.env.work.active t.1 b) = RegEncoding.toNat cfg.env.data.active b := by
  have hwrite :
      RegEncoding.toNat (cfg.env.data.grow 1).active
        (RegEncoding.writeNat cfg.env.work.active t.1 b) = RegEncoding.toNat (cfg.env.data.grow 1).active b :=
    RegEncoding.toNat_left_write_right
      (cfg.env.data.grow 1).active
      cfg.env.work.active
      (alg1_step2_dataCarry_work_disjoint cfg)
      b
      t.1
...
lemma alg1_step2_phase_normalization
  ...
  have hLne : (ASize ((cfg.env.data.grow 1).active) : ℝ) ≠ 0 :=
    ne_of_gt hLpos

  have hMneC : ((ASize cfg.env.work.active : ℝ) : ℂ) ≠ 0 := by
    exact_mod_cast hMne
...
lemma alg1_step2_actual_preIQFT_packet
    ...
    qs.eval
        ...
        (qs.eval
          (Gate.QFT cfg.env.circuit_workspace.step2Workspace.zExt)
          (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b))) = ∑ y : Fin (ASize ((cfg.env.data.grow 1).active)),
  ...
  have hxbase :
      RegEncoding.toNat ((cfg.env.data.grow 1).active)
        (RegEncoding.writeNat cfg.env.work.active t.1 b) = RegEncoding.toNat cfg.env.data.active b :=
  ...
  have hclean :
      cfg.env.circuit_workspace.step2Workspace.Clean
        (RegEncoding.writeNat xext.active y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) := by
    simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
  ...
  have hwork :
      RegEncoding.toNat cfg.env.work.active
        (RegEncoding.writeNat xext.active y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) = t.1 := by
    calc
      RegEncoding.toNat cfg.env.work.active
          (RegEncoding.writeNat xext.active y.1
            (RegEncoding.writeNat cfg.env.work.active t.1 b)) = RegEncoding.toNat cfg.env.work.active
        (RegEncoding.writeNat cfg.env.work.active t.1 b) := by
          exact
            RegEncoding.toNat_left_write_right
              cfg.env.work.active
  ...
  have hxext :
      RegEncoding.toNat ((cfg.env.data.grow 1).active)
        (RegEncoding.writeNat xext.active y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) = y.1 := by
    simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
      (RegEncoding.toNat_writeNat_of_lt
        xext.active
        y.1
  ...
  have hworkData :
      RegEncoding.toNat cfg.env.work.active
        (RegEncoding.writeNat ((cfg.env.data.grow 1).active) y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) = t.1 := by
    simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
      hwork
  ...
  simp [
    ...
    ExtReg.width,
    xext,
    ModMulCircuitWorkspaceOK.step2Workspace,
    PhaseProdWorkspace.zExt, ExtReg.withReserve,
    hxbase,
    hworkData,
    hxextData,
    mul_assoc
...
/--
Explicit Fourier-basis expansion of the ideal target packet immediately before
the inverse QFT.
-/
    ...
    qs.eval
        ...
        (qs.ket
          (RegEncoding.writeNat
            ...
            (RegEncoding.writeNat cfg.env.work.active t.1 b))) = ∑ y : Fin (ASize ((cfg.env.data.grow 1).active)),
  ...
  have hread :
      RegEncoding.toNat ((cfg.env.data.grow 1).active)
        (RegEncoding.writeNat
          ((cfg.env.data.grow 1).active)
          (alg1Step2Value cfg b)
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) = alg1Step2Value cfg b :=
    RegEncoding.toNat_writeNat_of_lt
      ((cfg.env.data.grow 1).active)
      (alg1Step2Value cfg b)
      (RegEncoding.writeNat cfg.env.work.active t.1 b)
  ...
  have hover :
      RegEncoding.writeNat
          ...
          (RegEncoding.writeNat
            ((cfg.env.data.grow 1).active)
            (alg1Step2Value cfg b)
            (RegEncoding.writeNat cfg.env.work.active t.1 b)) = RegEncoding.writeNat
        ((cfg.env.data.grow 1).active)
        y.1
        (RegEncoding.writeNat cfg.env.work.active t.1 b) :=
    writeNat_overwrite_same_reg
...
/--
Unitarity of the inverse QFT reduces a branch-error norm to the norm of the
difference between its actual and ideal pre-IQFT packets.
-/
lemma alg1_step2_branch_norm_eq_preIQFT_norm
    ...
    (t : Fin (ASize cfg.env.work.active)) :
    ‖qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
        (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)) - qs.ket
          (RegEncoding.writeNat
            ((cfg.env.data.grow 1).active)
            (alg1Step2Value cfg b)
            (RegEncoding.writeNat cfg.env.work.active t.1 b))‖ = ‖
      qs.eval
        ...
        (qs.eval
          (Gate.QFT cfg.env.circuit_workspace.step2Workspace.zExt)
          (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b))) - qs.eval
        (Gate.QFT cfg.env.circuit_workspace.step2Workspace.zExt)
        (qs.ket
          (RegEncoding.writeNat
            ((cfg.env.data.grow 1).active)
  ...
  have hU2 :
      qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
        (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)) = qs.eval (IQFT xext) source := by
    simp [ModMulConfig.U2, step2, alg1Step2Phase, xext, source, qs.eval_seq]

  have htarget :
      qs.eval (IQFT xext) targetFourier = qs.ket
        (RegEncoding.writeNat
          xext.active
          (alg1Step2Value cfg b)
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) := by
    ...
    ‖qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
        (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)) - qs.ket
          (RegEncoding.writeNat
            xext.active
            (alg1Step2Value cfg b)
            (RegEncoding.writeNat cfg.env.work.active t.1 b))‖ = ‖qs.eval (IQFT xext) source
...
/-- Split a QFT phase whose left argument is a sum into a product of phases. -/
        ...
        Complex.exp
          (Complex.I *
            (((2 * Real.pi / (L : ℝ)) *
                (r : ℝ) *
                (y : ℝ) : ℝ) : ℂ)) := by
  rw [qftPhase_eq_exp_I L (x + r) y hL, qftPhase_eq_exp_I L x y hL, ← Complex.exp_add]
  congr 1
  push_cast
  ring

/-- Bound the distance between two unit complex phases by their angle distance. -/
...
/--
Each actual Fourier coefficient differs from its ideal counterpart by at most
the normalized phase error induced by the Step-2 shift discrepancy.
-/
lemma alg1_step2_fourier_coeff_error_bound
    ...
    (b : qs.Basis)
    (t : Fin (ASize cfg.env.work.active))
    (_hb : GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag b)
    (ht : t ∈ alg1GoodLabels cfg b)
    (y : Fin (ASize ((cfg.env.data.grow 1).active))) :
    ‖alg1Step2ActualFourierCoeff cfg b t y
        - alg1Step2IdealFourierCoeff cfg b y‖ ≤ (2 * Real.pi * η) /
      Real.sqrt (ASize ((cfg.env.data.grow 1).active) : ℝ) := by
  let L : ℕ := ASize ((cfg.env.data.grow 1).active)
  ...
  calc
    ‖alg1Step2QFTScale cfg *
        qftPhase L x y.1 *
        Complex.exp (Complex.I * (θa : ℂ)) - alg1Step2QFTScale cfg *
          (qftPhase L x y.1 *
            Complex.exp (Complex.I * (θr : ℂ)))‖ = ‖(alg1Step2QFTScale cfg * qftPhase L x y.1) *
        (Complex.exp (Complex.I * (θa : ℂ))
          - Complex.exp (Complex.I * (θr : ℂ)))‖ := by
        congr 1
        ring
...
lemma alg1_step2_xext_fourier_label_injective
    ...
    (b : qs.Basis)
    (t : Fin (ASize cfg.env.work.active)) :
    ∀ y z : Fin (ASize ((cfg.env.data.grow 1).active)),
      RegEncoding.writeNat
          ((cfg.env.data.grow 1).active)
          y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b) = RegEncoding.writeNat
  ...
  apply Fin.ext
  calc
    y.1 = RegEncoding.toNat ((cfg.env.data.grow 1).active)
        (RegEncoding.writeNat
          ((cfg.env.data.grow 1).active)
          y.1
          (RegEncoding.writeNat cfg.env.work.active t.1 b)) := by
...
lemma alg1_step2_normalized_fourier_packet_bound
        ...
        alg1Step2IdealFourierCoeff cfg b y •
          qs.ket
            (RegEncoding.writeNat
              ((cfg.env.data.grow 1).active)
              y.1
              (RegEncoding.writeNat cfg.env.work.active t.1 b)) = P := by
    dsimp [P, δ, label, L]
    rw [← Finset.sum_sub_distrib]
    apply Finset.sum_congr rfl
    intro y hy
  ...
  have henergy : ‖P‖ ^ 2 ≤ C ^ 2 := by
    calc
      ‖P‖ ^ 2 = ∑ y : Fin L, ‖δ y • qs.ket (label y)‖ ^ 2 := hsq
...
lemma alg1_step2_single_label_fourier_stability
    ...
    ∃ Cbranch : ℝ, 0 ≤ Cbranch ∧
      ∀ (η : ℝ) (cfg : ModMulConfig η)
        ...
        ‖qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket
              (RegEncoding.writeNat cfg.env.work.active t.1 b)) - qs.ket
              (RegEncoding.writeNat
                ((cfg.env.data.grow 1).active)
                (alg1Step2Value cfg b)
                (RegEncoding.writeNat cfg.env.work.active t.1 b))‖
  ...
  · intro η cfg b t hb ht
    ...
    rw [alg1_step2_actual_preIQFT_packet qs cfg b t hb, alg1_step2_ideal_preIQFT_packet qs cfg b t hb]
    simpa using
...
/-- States supported on distinct work-register values are orthogonal. -/
private lemma alg1_work_sector_inner_zero
        ...
        β z •
          qs.ket
            (RegEncoding.writeNat xext z.1
              (RegEncoding.writeNat work u.1 b'))) = 0 := by
  classical
    ...
    calc
            ...
            (RegEncoding.writeNat work t.1 b)) := by
              ...
              calc
                RegEncoding.toNat work
                    (RegEncoding.writeNat xext y.1
                      (RegEncoding.writeNat work t.1 b)) = RegEncoding.toNat work
                    (RegEncoding.writeNat work t.1 b) :=
                  RegEncoding.toNat_left_write_right
                    work xext hWX
      ...
      _ = u.1 := by
              calc
                RegEncoding.toNat work
                    (RegEncoding.writeNat xext z.1
                      (RegEncoding.writeNat work u.1 b')) = RegEncoding.toNat work
  ...
  rw [inner_smul_left, inner_smul_right, qs.ket_inner_eq_zero_of_ne hlabel]
  simp

/--
The actual Step-2 branch before the inverse QFT remains entirely in the sector
selected by its work-register label.
-/
private lemma alg1_step2_preIQFT_work_packet
    ...
    (t : Fin (ASize cfg.env.work.active))
    (hb : GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag b) :
    ∃ α : Fin (ASize ((cfg.env.data.grow 1).active)) → ℂ,
      qs.eval
          (Gate.PhaseProdUsing
            (alg1Step2Phase cfg)
            cfg.env.work.active
  ...
  have hXW : Disjoint xext.active cfg.env.work.active := by
    simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
      alg1_step2_dataCarry_work_disjoint cfg

  have hWX : Disjoint cfg.env.work.active xext.active := by
    exact Shor.Disjoint.symm hXW

  refine ⟨
    fun y =>
      ((1 / Real.sqrt ((ASize xext.active : ℕ) : ℝ) : ℂ) *
        qftPhase
          (ASize xext.active)
  ...
  have hphase :
    ...
    have hwork :
      ...
      calc
        RegEncoding.toNat cfg.env.work.active
          ...
          RegEncoding.toNat_left_write_right
            cfg.env.work.active xext.active hWX base y.1
        _ = t.1 :=
          RegEncoding.toNat_writeNat_of_lt
            cfg.env.work.active t.1 b t.isLt
    ...
    have hworkData :
        RegEncoding.toNat cfg.env.work.active
          (RegEncoding.writeNat ((cfg.env.data.grow 1).active) y.1 base) = t.1 := by
      simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
        hwork

    have hdata :
        RegEncoding.toNat ((cfg.env.data.grow 1).active)
          (RegEncoding.writeNat ((cfg.env.data.grow 1).active) y.1 base) = y.1 := by
      simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
        hxext

    simpa [hworkData, hdata, xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
      (GateSemanticsFacts.eval_PhaseProdUsing_ket
        qs
        (alg1Step2Phase cfg)
        cfg.env.work.active
        ((cfg.env.data.grow 1).active)
        cfg.env.circuit_workspace.step2Workspace
        (RegEncoding.writeNat xext.active y.1 base)
        hclean)
  ...
  change
    qs.eval
        (Gate.PhaseProdUsing
          ...
          ((cfg.env.data.grow 1).active)
          cfg.env.circuit_workspace.step2Workspace)
        (qs.eval (Gate.QFT xext) (qs.ket base)) = _
  ...
  simp [xext, base, ASize, ExtReg.toNat, ExtReg.width, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve, smul_smul, mul_assoc]

/-- A QFT of a grown data-carry write remains in the same work sector. -/
private lemma alg1_qft_xext_work_packet
    ...
    (u : Fin (ASize work))
    (v : ℕ) :
    ∃ β : Fin (ASize xext.active) → ℂ,
      qs.eval
          (Gate.QFT xext)
          (qs.ket
            (RegEncoding.writeNat xext.active v
              (RegEncoding.writeNat work u.1 b))) = ∑ z : Fin (ASize xext.active),
        β z •
          qs.ket
            (RegEncoding.writeNat xext.active z.1
              (RegEncoding.writeNat work u.1 b)) := by
  classical
  ...
  refine ⟨
    fun z =>
      (1 / Real.sqrt (((2 ^ xext.width) : ℕ) : ℝ) : ℂ) *
        qftPhase
          (2 ^ xext.width)
          (RegEncoding.toNat xext.active base)
          z.1,
...
private lemma alg1_step2_actual_ref_work_cross_zero
    ...
    inner ℂ
      ...
      (qs.ket
        (RegEncoding.writeNat
          ((cfg.env.data.grow 1).active)
          (alg1Step2Value cfg b')
          (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = 0 := by
  classical
  ...
  have hU2 :
      qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
        (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)) = qs.eval (IQFT xext)
        (qs.eval
          (Gate.PhaseProdUsing
            (alg1Step2Phase cfg)
            cfg.env.work.active
        ...
        (qs.eval
          ...
          (qs.ket
            (RegEncoding.writeNat
              xext.active
              (alg1Step2Value cfg b')
              (RegEncoding.writeNat cfg.env.work.active u.1 b')))) = qs.ket
        (RegEncoding.writeNat
          xext.active
          (alg1Step2Value cfg b')
          (RegEncoding.writeNat cfg.env.work.active u.1 b')) := by
  ...
  rcases alg1_step2_preIQFT_work_packet qs cfg b t hb with
    ⟨α, hα⟩

  rcases
      alg1_qft_xext_work_packet
        qs xext cfg.env.work.active b' u
    ...
    inner ℂ
        ...
        (qs.ket
          (RegEncoding.writeNat
            xext.active
            (alg1Step2Value cfg b')
            (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = inner ℂ
      (qs.eval (IQFT xext)
        (qs.eval
          (Gate.PhaseProdUsing
            (alg1Step2Phase cfg)
    ...
    _ = 0 := by
      ...
      exact
        alg1_work_sector_inner_zero
          qs xext.active cfg.env.work.active hXW b b' t u htu α β

/-- Step-2 branch errors belonging to distinct work labels are orthogonal. -/
lemma alg1_step2_error_work_orthogonal
    ...
    inner ℂ
      ...
      (alg1Step2Error qs cfg b' u) = 0 := by
  classical
  ...
  have href_ne :
      RegEncoding.writeNat xext.active
          (alg1Step2Value cfg b)
          (RegEncoding.writeNat cfg.env.work.active t.1 b)
        ≠
      RegEncoding.writeNat xext.active
            ...
            (RegEncoding.writeNat cfg.env.work.active t.1 b)) := by
              ...
              calc
                RegEncoding.toNat cfg.env.work.active
                    ...
                    (RegEncoding.writeNat cfg.env.work.active t.1 b) :=
                  RegEncoding.toNat_left_write_right
                    cfg.env.work.active xext.active hWX
                    (RegEncoding.writeNat cfg.env.work.active t.1 b)
                    (alg1Step2Value cfg b)
                _ = t.1 :=
                  RegEncoding.toNat_writeNat_of_lt
      ...
      _ = u.1 := by
              calc
                RegEncoding.toNat cfg.env.work.active
                    (RegEncoding.writeNat xext.active
                      (alg1Step2Value cfg b')
                      (RegEncoding.writeNat cfg.env.work.active u.1 b')) = RegEncoding.toNat cfg.env.work.active
                    (RegEncoding.writeNat cfg.env.work.active u.1 b') :=
                  RegEncoding.toNat_left_write_right
                    cfg.env.work.active xext.active hWX
                    (RegEncoding.writeNat cfg.env.work.active u.1 b')
  ...
  have hactual_actual :
      inner ℂ
        (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
          (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)))
        (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
          (qs.ket (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = 0 := by
    calc
      inner ℂ
          (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b)))
          (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = inner ℂ
        (qs.ket (RegEncoding.writeNat cfg.env.work.active t.1 b))
        (qs.ket (RegEncoding.writeNat cfg.env.work.active u.1 b')) :=
      qs.inner_preserved _ _ _
    _ = 0 :=
  ...
  have href_ref :
      inner ℂ
        ...
        (qs.ket
          (RegEncoding.writeNat xext.active
            (alg1Step2Value cfg b')
            (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = 0 :=
    ...
    have hswap_xext :
        inner ℂ
          ...
          (qs.ket
            (RegEncoding.writeNat xext.active
              (alg1Step2Value cfg b)
              (RegEncoding.writeNat cfg.env.work.active t.1 b))) = 0 := by
      simpa [xext, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve] using
        hswap
      ...
      inner ℂ
          (qs.ket
            (RegEncoding.writeNat xext.active
              ...
              (RegEncoding.writeNat cfg.env.work.active t.1 b)))
          (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket (RegEncoding.writeNat cfg.env.work.active u.1 b'))) = star
        (inner ℂ
          (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket (RegEncoding.writeNat cfg.env.work.active u.1 b')))
          (qs.ket
  ...
  have href_ref_data :
      inner ℂ
        (qs.ket
          (RegEncoding.writeNat ((cfg.env.data.grow 1).active)
            (alg1Step2Value cfg b)
            (RegEncoding.writeNat cfg.env.work.active t.1 b)))
        (qs.ket
          (RegEncoding.writeNat ((cfg.env.data.grow 1).active)
...
lemma alg1_step2_source_label_injective_of_good
    ...
    (hgood : ∀ i ∈ S, GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag i.1) :
    ∀ i ∈ S, ∀ j ∈ S, i ≠ j →
      RegEncoding.writeNat cfg.env.work.active i.2.1 i.1
        ≠
...
/--
For a finite family of good inputs, the squared norm of the written source
packet equals the sum of the squared input coefficients.
-/
lemma alg1_step2_fixed_work_source_energy
    ...
    ‖∑ i ∈ S,
        α i •
          qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)‖ ^ 2 = ∑ i ∈ S, ‖α i‖ ^ 2 := by
  classical
  ...
  have horth :
      ∀ i ∈ S, ∀ j ∈ S, i ≠ j →
        inner ℂ
          ...
          (α j •
            qs.ket
              (RegEncoding.writeNat cfg.env.work.active j.2.1 j.1)) = 0 := by
    intro i hi j hj hij
    rw [inner_smul_left, inner_smul_right, qs.ket_inner_eq_zero_of_ne (hinj i hi j hj hij)]
    simp
  ...
  calc
    ‖∑ i ∈ S,
        α i •
          qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)‖ ^ 2 = ∑ i ∈ S,
      ‖α i •
        qs.ket
          (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)‖ ^ 2 := hsq
    _ =
    ∑ i ∈ S, ‖α i‖ ^ 2 := by
      apply Finset.sum_congr rfl
      intro i hi
...
private lemma alg1_qftPhase_add_left
    (L x r y : ℕ) :
    qftPhase L (x + r) y = qftPhase L x y * qftPhase L r y := by
...
labels scales the squared packet norm by at most the squared factor bound.
-/
private lemma norm_sq_sum_ket_mul_le_of_label_constant
    ...
    (hC : 0 ≤ C)
    (hbound : ∀ i ∈ s, ‖L i‖ ≤ C)
    (hconstant : ∀ i ∈ s, ∀ j ∈ s, label i = label j → L i = L j) :
    ‖∑ i ∈ s, (a i * L i) • qs.ket (label i)‖ ^ 2 ≤ C ^ 2 *
  ...
  · rcases hs with ⟨i0, hi0⟩
    ...
    have hterm :
      ...
      calc
        ‖(A b * μ b) • qs.ket b‖ ^ 2 = ‖A b * μ b‖ ^ 2 := by
            simp [norm_smul, ket_norm_one qs]
        _ =
          ‖A b‖ ^ 2 * ‖μ b‖ ^ 2 := by
            rw [norm_mul]
    ...
    calc
      ‖∑ i ∈ s, (a i * L i) • qs.ket (label i)‖ ^ 2 = ‖∑ b ∈ B, (A b * μ b) • qs.ket b‖ ^ 2 := by
          rw [hmult_group]
      _ =
...
/-- Equal Fourier labels receive equal fixed-residue error multipliers. -/
private lemma alg1_step2_fourier_multiplier_constant_on_labels
    ...
    (hlabel : alg1Step2FourierLabel qs cfg p = alg1Step2FourierLabel qs cfg q) :
    alg1Step2FourierMultiplier qs cfg r p = alg1Step2FourierMultiplier qs cfg r q := by
  have hy : p.2.1 = q.2.1 := by
    calc
      p.2.1 = RegEncoding.toNat
          ((cfg.env.data.grow 1).active)
      ...
      _ = q.2.1 :=
        RegEncoding.toNat_writeNat_of_lt
          ((cfg.env.data.grow 1).active)
          q.2.1
          (RegEncoding.writeNat cfg.env.work.active q.1.2.1 q.1.1)
          q.2.isLt

  have hpwork : p.1.2.1 = q.1.2.1 := by
    have hXW : Disjoint ((cfg.env.data.grow 1).active) cfg.env.work.active :=
      alg1_step2_dataCarry_work_disjoint cfg
    have hWX : Disjoint cfg.env.work.active ((cfg.env.data.grow 1).active) :=
      Shor.Disjoint.symm hXW
    calc
      p.1.2.1 = RegEncoding.toNat
          cfg.env.work.active
          (alg1Step2FourierLabel qs cfg p) := by
            symm
            calc
              RegEncoding.toNat
                  cfg.env.work.active
                  (alg1Step2FourierLabel qs cfg p) = RegEncoding.toNat
                  cfg.env.work.active
                  (RegEncoding.writeNat cfg.env.work.active p.1.2.1 p.1.1) := by
                    exact
                      RegEncoding.toNat_left_write_right
                        cfg.env.work.active
                        ((cfg.env.data.grow 1).active)
      ...
      _ = q.1.2.1 := by
            calc
              RegEncoding.toNat
                  cfg.env.work.active
                  (alg1Step2FourierLabel qs cfg q) = RegEncoding.toNat
                  cfg.env.work.active
                  (RegEncoding.writeNat cfg.env.work.active q.1.2.1 q.1.1) := by
                    exact
                      RegEncoding.toNat_left_write_right
                        cfg.env.work.active
                        ((cfg.env.data.grow 1).active)
  ...
  have hp' :
      p ∈ S.sigma
        (fun _ : Alg1Step2SourceIndex qs cfg =>
          (Finset.univ :
            Finset (Fin (ASize ((cfg.env.data.grow 1).active))))) := by
...
/--
Flatten the QFT of a fixed-work source packet into its indexed Fourier packet.
-/
private lemma alg1_step2_fixed_work_qft_packet
    ...
    (hgood : ∀ i ∈ S, GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag i.1) :
    qs.eval (Gate.QFT cfg.env.circuit_workspace.step2Workspace.zExt)
      (∑ i ∈ S,
        α i •
          qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) = ∑ p ∈ alg1Step2FourierIndices qs cfg S,
  ...
  have hqft :
      ∀ i ∈ S,
        qs.eval (Gate.QFT xext)
          (qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) = ∑ y : Fin (ASize xext.active),
          (alg1Step2QFTScale cfg *
            qftPhase
              (ASize xext.active)
              (RegEncoding.toNat cfg.env.data.active i.1)
              y.1) •
            qs.ket
              (RegEncoding.writeNat
                xext.active
    ...
    have hxdata :
        RegEncoding.toNat ((cfg.env.data.grow 1).active)
          (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1) = RegEncoding.toNat cfg.env.data.active i.1 :=
    ...
    intro y hy
    rw [smul_smul]
    simp [xext, alg1Step2QFTScale, ASize, ExtReg.toNat, ExtReg.width, ModMulCircuitWorkspaceOK.step2Workspace, PhaseProdWorkspace.zExt, ExtReg.withReserve, hxdata,]
  ...
  calc
    qs.eval (Gate.QFT xext)
        (∑ i ∈ S,
          α i •
            qs.ket
              (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) = ∑ i ∈ S,
      α i •
        qs.eval (Gate.QFT xext)
          (qs.ket
            (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) := by
...
/--
Represent one Step-2 branch error as the inverse QFT of its Fourier-coefficient
difference packet.
-/
private lemma alg1_step2_branch_error_eq_iqft_packet
    ...
    (t : Fin (ASize cfg.env.work.active))
    (hb : GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag b) :
    alg1Step2Error qs cfg b t = qs.eval (IQFT cfg.env.circuit_workspace.step2Workspace.zExt)
      (∑ y : Fin (ASize ((cfg.env.data.grow 1).active)),
        (alg1Step2ActualFourierCoeff cfg b t y - alg1Step2IdealFourierCoeff cfg b y) •
          qs.ket
            (RegEncoding.writeNat
              ((cfg.env.data.grow 1).active)
              y.1
  ...
  have hactual :
      actualPre = ∑ y : Fin (ASize xext.active),
        alg1Step2ActualFourierCoeff cfg b t y •
          qs.ket
            (RegEncoding.writeNat xext.active y.1 source) := by
    simpa [actualPre, xext, source] using
      alg1_step2_actual_preIQFT_packet qs cfg b t hb

  have hideal :
      idealPre = ∑ y : Fin (ASize xext.active),
        alg1Step2IdealFourierCoeff cfg b y •
          qs.ket
            (RegEncoding.writeNat xext.active y.1 source) := by
  ...
  calc
    alg1Step2Error qs cfg b t = qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg) (qs.ket source)
        - qs.ket target := by
          rfl
    _ =
    ...
    _ =
      qs.eval (IQFT xext)
        ((∑ y : Fin (ASize xext.active),
          alg1Step2ActualFourierCoeff cfg b t y •
            qs.ket
              (RegEncoding.writeNat xext.active y.1 source)) - ∑ y : Fin (ASize xext.active),
          alg1Step2IdealFourierCoeff cfg b y •
            qs.ket
              (RegEncoding.writeNat xext.active y.1 source)) := by
          rw [hactual, hideal]
    _ =
      qs.eval (IQFT xext)
        (∑ y : Fin (ASize xext.active),
          ...
          rw [← sub_smul]
    _ =
      qs.eval (IQFT cfg.env.circuit_workspace.step2Workspace.zExt)
        (∑ y : Fin (ASize ((cfg.env.data.grow 1).active)),
          (alg1Step2ActualFourierCoeff cfg b t y - alg1Step2IdealFourierCoeff cfg b y) •
            qs.ket
              (RegEncoding.writeNat
...
private lemma alg1_step2_fixed_work_coeff_factor
    ...
    (y : Fin (ASize ((cfg.env.data.grow 1).active))) :
    α i *
        (alg1Step2ActualFourierCoeff cfg i.1 i.2 y - alg1Step2IdealFourierCoeff cfg i.1 y) = alg1Step2FourierBaseCoeff qs cfg α ⟨i, y⟩ *
...
/--
Represent the coherent error of a fixed work fiber as the inverse QFT of a
packet obtained by multiplying its base coefficients by bounded phase errors.
-/
lemma alg1_step2_fixed_work_error_eq_iqft_multiplier_packet
    ...
    (hfixed :
      ∀ i ∈ S,
        ...
        i.2 ∈ alg1GoodLabels cfg i.1 ∧
        alg1TargetResidue cfg i.1 = r) :
    ∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2 = qs.eval (IQFT cfg.env.circuit_workspace.step2Workspace.zExt)
      (∑ p ∈ alg1Step2FourierIndices qs cfg S,
        (alg1Step2FourierBaseCoeff qs cfg α p * alg1Step2FourierMultiplier qs cfg r p) •
          qs.ket (alg1Step2FourierLabel qs cfg p)) := by
  classical
  ...
  let packet : Alg1Step2SourceIndex qs cfg → qs.State :=
    fun i =>
      ∑ y : Fin (ASize xext.active),
        (alg1Step2ActualFourierCoeff cfg i.1 i.2 y - alg1Step2IdealFourierCoeff cfg i.1 y) •
          qs.ket
            (RegEncoding.writeNat
              xext.active
              y.1
  ...
  calc
    ∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2 = ∑ i ∈ S,
...
lemma alg1_step2_fixed_work_fourier_contraction
    ...
    ∀ (η : ℝ) (cfg : ModMulConfig η)
      ...
      (∀ i ∈ S,
        ...
        i.2 ∈ alg1GoodLabels cfg i.1 ∧
        alg1TargetResidue cfg i.1 = r) →
      ‖∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2‖ ^ 2 ≤ (2 * Real.pi * η) ^ 2 *
        ‖∑ i ∈ S,
          α i •
            qs.ket
  ...
  have hconst :
      ∀ p ∈ alg1Step2FourierIndices qs cfg S,
        ∀ q ∈ alg1Step2FourierIndices qs cfg S,
          alg1Step2FourierLabel qs cfg p = alg1Step2FourierLabel qs cfg q →
          alg1Step2FourierMultiplier qs cfg r p = alg1Step2FourierMultiplier qs cfg r q := by
    intro p hp q hq hpq
  ...
  have hdiag :
      ‖∑ p ∈ alg1Step2FourierIndices qs cfg S,
          (alg1Step2FourierBaseCoeff qs cfg α p * alg1Step2FourierMultiplier qs cfg r p) •
            qs.ket (alg1Step2FourierLabel qs cfg p)‖ ^ 2 ≤ (2 * Real.pi * η) ^ 2 *
        ‖∑ p ∈ alg1Step2FourierIndices qs cfg S,
          alg1Step2FourierBaseCoeff qs cfg α p •
  ...
  calc
    ‖∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2‖ ^ 2 = ‖qs.eval (IQFT cfg.env.circuit_workspace.step2Workspace.zExt)
        (∑ p ∈ alg1Step2FourierIndices qs cfg S,
          (alg1Step2FourierBaseCoeff qs cfg α p * alg1Step2FourierMultiplier qs cfg r p) •
...
lemma alg1_step2_fixed_work_packet_sq_bound
    ...
    ∀ (η : ℝ) (cfg : ModMulConfig η)
      ...
      (∀ i ∈ S,
        ...
        i.2 ∈ alg1GoodLabels cfg i.1 ∧
        alg1TargetResidue cfg i.1 = r) →
      ‖∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2‖ ^ 2 ≤ (2 * Real.pi ^ 2 * η) *
        ∑ i ∈ S, ‖α i‖ ^ 2 := by
  intro η cfg S α t r hfixed
  ...
  have hcontract :
      ‖∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2‖ ^ 2 ≤ (2 * Real.pi * η) ^ 2 *
        ‖∑ i ∈ S,
          α i •
            qs.ket
              (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)‖ ^ 2 :=
...
/-! ## Work-Fiber Reassembly and Coherent Operator Bound -/

/--
Apply the fixed-work packet estimate to one fiber of an arbitrary good coherent
packet, using residue agreement within that fiber.
-/
lemma alg1_step2_work_fiber_sq_bound
    ...
    ‖∑ i ∈ S.filter (fun i => i.2 = t),
        α i • alg1Step2Error qs cfg i.1 i.2‖ ^ 2 ≤ (2 * Real.pi ^ 2 * η) *
...
lemma alg1_step2_error_eq_sum_work_fibers
    ...
    (S : Finset (Σ _b : qs.Basis, Fin (ASize cfg.env.work.active)))
    (α : (Σ _b : qs.Basis, Fin (ASize cfg.env.work.active)) → ℂ) :
    ∑ i ∈ S, α i • alg1Step2Error qs cfg i.1 i.2 = ∑ t : Fin (ASize cfg.env.work.active),
      ∑ i ∈ S.filter (fun i => i.2 = t),
  ...
  calc
    ∑ t : Fin (ASize cfg.env.work.active),
      ∑ i ∈ S.filter (fun i => i.2 = t),
        α i • alg1Step2Error qs cfg i.1 i.2 = ∑ t : Fin (ASize cfg.env.work.active),
        ∑ i ∈ S,
          if i.2 = t then
...
lemma alg1_step2_work_fiber_orthogonal
    ...
    (α : (Σ _b : qs.Basis, Fin (ASize cfg.env.work.active)) → ℂ)
    (hgood : ∀ i ∈ S, GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag i.1) :
    ∀ t ∈ (Finset.univ : Finset (Fin (ASize cfg.env.work.active))),
      ∀ u ∈ (Finset.univ : Finset (Fin (ASize cfg.env.work.active))),
        t ≠ u →
        inner ℂ
          (∑ i ∈ S.filter (fun i => i.2 = t),
            α i • alg1Step2Error qs cfg i.1 i.2)
          (∑ j ∈ S.filter (fun j => j.2 = u),
            α j • alg1Step2Error qs cfg j.1 j.2) = 0 := by
  intro t _ u _ htu
  rw [inner_sum]
  refine Finset.sum_eq_zero ?_
  intro j hj
  ...
  rw [hit, hju]
  simp [alg1_step2_error_work_orthogonal qs cfg i.1 j.1 t u (hgood i hiS) (hgood j hjS) htu]
...
lemma alg1_step2_energy_eq_sum_work_fibers
    ...
    (α : (Σ _b : qs.Basis, Fin (ASize cfg.env.work.active)) → ℂ) :
    ∑ t : Fin (ASize cfg.env.work.active),
      ∑ i ∈ S.filter (fun i => i.2 = t), ‖α i‖ ^ 2 = ∑ i ∈ S, ‖α i‖ ^ 2 := by
  classical
  calc
    ∑ t : Fin (ASize cfg.env.work.active),
      ∑ i ∈ S.filter (fun i => i.2 = t), ‖α i‖ ^ 2 = ∑ t : Fin (ASize cfg.env.work.active),
        ∑ i ∈ S,
...
coherent packet by `η` times its coefficient energy. Orthogonality between work
fibers allows the fixed-fiber estimates to add without cross terms.
-/
lemma alg1_step2_good_packet_operator_sq_bound
  ...
  let E : Fin (ASize cfg.env.work.active) → qs.State :=
    fun t =>
      ∑ i ∈ S.filter (fun i => i.2 = t),
        α i • alg1Step2Error qs cfg i.1 i.2
  ...
  have hsplit :
      ∑ i ∈ S,
        α i •
          (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
            (qs.ket (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) - qs.ket
              (RegEncoding.writeNat
                ((cfg.env.data.grow 1).active)
                (alg1Step2Value cfg i.1)
                (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1))) = ∑ t : Fin (ASize cfg.env.work.active), E t := by
  ...
  have horth :
    ...
    simpa [E] using
      alg1_step2_work_fiber_orthogonal qs cfg S α (fun i hi => (hgood i hi).1)

  have hparseval :
      ‖∑ t : Fin (ASize cfg.env.work.active), E t‖ ^ 2 = ∑ t : Fin (ASize cfg.env.work.active), ‖E t‖ ^ 2 :=
    alg1_step2_norm_sq_sum_eq_sum_norm_sq_of_orthogonal
      (qs := qs)
      (Finset.univ : Finset (Fin (ASize cfg.env.work.active)))
      E
      horth
  ...
  have hsum :
      ∑ t : Fin (ASize cfg.env.work.active), ‖E t‖ ^ 2 ≤ ∑ t : Fin (ASize cfg.env.work.active),
        (2 * Real.pi ^ 2 * η) *
          ∑ i ∈ S.filter (fun i => i.2 = t), ‖α i‖ ^ 2 := by
  ...
  have hfactor :
      (∑ t : Fin (ASize cfg.env.work.active),
        (2 * Real.pi ^ 2 * η) *
          ∑ i ∈ S.filter (fun i => i.2 = t), ‖α i‖ ^ 2) = (2 * Real.pi ^ 2 * η) *
        ∑ i ∈ S, ‖α i‖ ^ 2 := by
    rw [← Finset.mul_sum]
        ...
        (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
          (qs.ket (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)) - qs.ket
            (RegEncoding.writeNat
              ((cfg.env.data.grow 1).active)
              (alg1Step2Value cfg i.1)
              (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)))‖ ^ 2 = ‖∑ t : Fin (ASize cfg.env.work.active), E t‖ ^ 2 := by
        rw [hsplit]
    _ =
      ∑ t : Fin (ASize cfg.env.work.active), ‖E t‖ ^ 2 := hparseval
    _ ≤
      ∑ t : Fin (ASize cfg.env.work.active),
        (2 * Real.pi ^ 2 * η) *
...
/--
One nonnegative constant simultaneously bounds the error of every retained
basis/work branch and the squared Step-2 error of every valid unit-state trace.
            ...
            (qs.eval (ModMulConfig.U2 (Basis := qs.Basis) cfg)
              (qs.ket
                ...
                (RegEncoding.writeNat
                  ((cfg.env.data.grow 1).active)
                  (alg1Step2Value cfg i.1)
                  (RegEncoding.writeNat cfg.env.work.active i.2.1 i.1)))‖ ^ 2 ≤ (Ccoh * η) * ∑ i ∈ Sgood, ‖α i‖ ^ 2 :=
