...
lemma FreshZero.of_subset
  ...
  · let iSmall : Fin (regSize small) := ⟨j, hj⟩
    let q : ℕ := small.get iSmall
    have hqSmall : q ∈ small.qubits := by
      dsimp [q, iSmall, Reg.get]
      exact List.get_mem small.qubits _
    have hqBig : q ∈ big.qubits := hsub q hqSmall
    let jBig : ℕ := big.qubits.idxOf q
    have hjBig : jBig < big.qubits.length := by
      exact List.idxOf_lt_length_of_mem hqBig
    let iBig : Fin (regSize big) :=
      ⟨jBig, by simpa [jBig, regSize, Reg.width] using hjBig⟩
    have hgetBig : big.get iBig = q := by
      simp [Reg.get, iBig, jBig, q, List.getElem_idxOf]
    calc
      Nat.testBit (RegEncoding.toNat small b) j
          = RegEncoding.bit q b := by
            symm
            simpa [q, iSmall] using RegEncoding.bit_eq_testBit_toNat small b iSmall
      _ = RegEncoding.bit (big.get iBig) b := by rw [hgetBig]
      _ = Nat.testBit (RegEncoding.toNat big b) iBig.1 :=
        RegEncoding.bit_eq_testBit_toNat big b iBig
      _ = false := by
        rw [hzero]
        simp
  · have hwidth : regSize small ≤ j := Nat.le_of_not_gt hj
    have hToNat : RegEncoding.toNat small b < 2 ^ regSize small := by
      simpa [ASize] using RegEncoding.toNat_lt_ASize (r := small) (b := b)
    have hpow : 2 ^ regSize small ≤ 2 ^ j := by
      exact Nat.pow_le_pow_right (by omega) hwidth
    exact Nat.testBit_eq_false_of_lt (lt_of_lt_of_le hToNat hpow)
...
lemma PhaseSplitLayout.child_newBits_subset_parent_reserve
  ...
  have hqChildReserve : qbit ∈ (layout.reserve i).qubits := by
    apply List.mem_of_mem_take
    simpa [PhaseSplitLayout.child, ExtReg.newBits, ExtReg.withReserve, Reg.take] using hq
  have hqFlatten :
      qbit ∈ (List.ofFn fun j : Fin k => (layout.reserve j).qubits).flatten := by
    rw [List.mem_flatten]
    refine ⟨(layout.reserve i).qubits, ?_, hqChildReserve⟩
    simp
...
lemma freshZero_reserve_of_freshFor_capacity
    ...
    (e : ExtReg)
    (b : Basis)
    (hclean : ExtReg.FreshFor e e.capacity b) :
    FreshZero e.reserve b := by
  have hnewBits : e.newBits e.capacity = e.reserve := by
    cases e with
    | mk active reserve hdisj =>
      cases reserve with
      | mk qubits nodup =>
        simp [ExtReg.newBits, ExtReg.capacity, Reg.take, regSize, Reg.width]
  simpa [ExtReg.FreshFor, hnewBits] using hclean
...
lemma compilerWorkspaceOK_of_recursiveWorkspaceCleanBasis
  ...
  have hxReserve : FreshZero x.reserve b := by
    exact freshZero_reserve_of_freshFor_capacity (e := x) (b := b) hclean.1
  have hzReserve : FreshZero z.reserve b := by
    exact freshZero_reserve_of_freshFor_capacity (e := z) (b := b) hclean.2
  change src.CanGrowTo Wwork ∧ src.CleanForGrowth Wwork b
  constructor
  · simpa [src, need, Wwork, LayoutState.CanGrowToNeeds] using hcapacity
  · constructor
    · intro i
      change ExtReg.FreshFor (layout.xSplit.child i) (Wwork - (layout.xSplit.child i).width) b
      unfold ExtReg.FreshFor
      apply FreshZero.of_subset
        (small := (layout.xSplit.child i).newBits (Wwork - (layout.xSplit.child i).width))
        (big := x.reserve)
        (b := b)
      · exact
          PhaseSplitLayout.child_newBits_subset_parent_reserve layout.xSplit i
            (Wwork - (layout.xSplit.child i).width)
      · exact hxReserve
    · intro i
      change ExtReg.FreshFor (layout.zSplit.child i) (Wwork - (layout.zSplit.child i).width) b
      unfold ExtReg.FreshFor
      apply FreshZero.of_subset
        (small := (layout.zSplit.child i).newBits (Wwork - (layout.zSplit.child i).width))
        (big := z.reserve)
        (b := b)
      · exact
          PhaseSplitLayout.child_newBits_subset_parent_reserve layout.zSplit i
            (Wwork - (layout.zSplit.child i).width)
      · exact hzReserve
...
lemma eval_compileSignedAllocations_ket_fits_and_child_clean
    ...
    ∃ bAlloc : qs.Basis,
      ...
      ∀ b' : qs.Basis,
        ...
        ∀ i : Fin k, RecursiveWorkspaceCleanBasis (dst.xslot i) (dst.zslot i) b' := by
  let src : LayoutState k := initSignedLayoutState layout
  let need : NeededWidths k := scanNeededWidths x z ops
  ...
  change
    ∃ bAlloc : qs.Basis,
      ...
      ∀ b' : qs.Basis,
        SameOutsideLayout qs dst bAlloc b' →
        ∀ i : Fin k, RecursiveWorkspaceCleanBasis (dst.xslot i) (dst.zslot i) b'
  rcases eval_compileSignedAllocations_ket_fits (qs := qs) (layout := layout) (ops := ops)
      (b := b) hworkspace with
    ⟨bAlloc, hEval, hEnc⟩
  rcases eval_compileSignedAllocations_sameOutside (qs := qs) (layout := layout) (ops := ops)
      (b := b) hworkspace with
    ⟨bOutside, hEvalOutside, hOutside⟩
  have hbOutside : bOutside = bAlloc := by
    apply qs.ket_inj
    rw [← hEval, ← hEvalOutside]
  subst bOutside
  have hOwned : dst.OwnedPairwiseDisjoint := by
    simpa [src, dst, need] using targetSignedLayoutState_owned_disjoint layout need
  have hxRootZero : FreshZero x.reserve b := freshZero_reserve_of_freshFor_capacity x b hclean.1
  have hzRootZero : FreshZero z.reserve b := freshZero_reserve_of_freshFor_capacity z b hclean.2
  have outsideXReserve :
      ∀ (i : Fin k) (qbit : ℕ),
        qbit ∈ (dst.xslot i).reserve.qubits → OutsideLayout dst (ExtReg.ofReg (qubitReg qbit)) := by
    intro i qbit hqReserve
    constructor
    · intro j
      rw [ExtReg.ActiveDisjoint, Disjoint, List.disjoint_left]
      intro q hqSingleton hqActive
      simp [ExtReg.ofReg, qubitReg, Reg.singleton] at hqSingleton
      subst q
      by_cases hij : i = j
      · subst j
        exact (dst.xslot i).active_reserve_disjoint hqActive hqReserve
      · exact hOwned.1 i j hij (List.mem_append_right _ hqReserve) (List.mem_append_left _ hqActive)
    · intro j
      rw [ExtReg.ActiveDisjoint, Disjoint, List.disjoint_left]
      intro q hqSingleton hqActive
      simp [ExtReg.ofReg, qubitReg, Reg.singleton] at hqSingleton
      subst q
      exact hOwned.2.2 i j (List.mem_append_right _ hqReserve) (List.mem_append_left _ hqActive)
  have outsideZReserve :
      ∀ (i : Fin k) (qbit : ℕ),
        qbit ∈ (dst.zslot i).reserve.qubits → OutsideLayout dst (ExtReg.ofReg (qubitReg qbit)) := by
    intro i qbit hqReserve
    constructor
    · intro j
      rw [ExtReg.ActiveDisjoint, Disjoint, List.disjoint_left]
      ...
      simp [ExtReg.ofReg, qubitReg, Reg.singleton] at hqSingleton
      subst q
      exact hOwned.2.2 j i (List.mem_append_left _ hqActive) (List.mem_append_right _ hqReserve)
    · intro j
      rw [ExtReg.ActiveDisjoint, Disjoint, List.disjoint_left]
      intro q hqSingleton hqActive
      simp [ExtReg.ofReg, qubitReg, Reg.singleton] at hqSingleton
      ...
      by_cases hij : i = j
      · subst j
        exact (dst.zslot i).active_reserve_disjoint hqActive hqReserve
      · exact hOwned.2.1 i j hij (List.mem_append_right _ hqReserve) (List.mem_append_left _ hqActive)
  refine ⟨bAlloc, hEval, hEnc, ?_⟩
  intro b' hAllocOutside i
  have hOutsideTotal : SameOutsideLayout qs dst b b' :=
    SameOutsideLayout.trans (qs := qs) hOutside hAllocOutside
  constructor
  · unfold ExtReg.FreshFor
    have hZeroInOriginal : FreshZero ((dst.xslot i).newBits (dst.xslot i).capacity) b := by
      apply FreshZero.of_subset
        (((dst.xslot i).newBits (dst.xslot i).capacity)) x.reserve b ?_ hxRootZero
      intro qbit hqNew
      have hqTargetReserve : qbit ∈ (dst.xslot i).reserve.qubits := by
        exact List.mem_of_mem_take hqNew
      have hqDropped :
          qbit ∈
            List.drop (commonNeededWidth need - (layout.xSplit.child i).width)
              (layout.xSplit.child i).reserve.qubits := by
        simpa [dst, src, targetSignedLayoutState, growExtRegTo, ExtReg.grow,
          ExtReg.remainingReserve, Reg.drop] using hqTargetReserve
      have hqChildReserve : qbit ∈ (layout.xSplit.child i).reserve.qubits :=
        List.mem_of_mem_drop hqDropped
      apply
        PhaseSplitLayout.child_newBits_subset_parent_reserve layout.xSplit i
          (layout.xSplit.child i).capacity
      simpa [ExtReg.newBits, ExtReg.capacity, Reg.take, regSize, Reg.width] using hqChildReserve
    apply
      FreshZero.of_eq_on_bits ((dst.xslot i).newBits (dst.xslot i).capacity) b b' ?_
        hZeroInOriginal
    intro qbit hqNew
    have hqTargetReserve : qbit ∈ (dst.xslot i).reserve.qubits := List.mem_of_mem_take hqNew
    exact SameOutsideLayout.bit_eq_of_outside qs hOutsideTotal qbit
      (outsideXReserve i qbit hqTargetReserve)
  · unfold ExtReg.FreshFor
    have hZeroInOriginal : FreshZero ((dst.zslot i).newBits (dst.zslot i).capacity) b := by
      apply FreshZero.of_subset
        (((dst.zslot i).newBits (dst.zslot i).capacity)) z.reserve b ?_ hzRootZero
      intro qbit hqNew
      have hqTargetReserve : qbit ∈ (dst.zslot i).reserve.qubits := by
        exact List.mem_of_mem_take hqNew
      have hqDropped :
          qbit ∈
            List.drop (commonNeededWidth need - (layout.zSplit.child i).width)
              (layout.zSplit.child i).reserve.qubits := by
        simpa [dst, src, targetSignedLayoutState, growExtRegTo, ExtReg.grow,
          ExtReg.remainingReserve, Reg.drop] using hqTargetReserve
      have hqChildReserve : qbit ∈ (layout.zSplit.child i).reserve.qubits :=
        List.mem_of_mem_drop hqDropped
      apply
        PhaseSplitLayout.child_newBits_subset_parent_reserve layout.zSplit i
      ...
      simpa [ExtReg.newBits, ExtReg.capacity, Reg.take, regSize, Reg.width] using hqChildReserve
    apply
      FreshZero.of_eq_on_bits ((dst.zslot i).newBits (dst.zslot i).capacity) b b' ?_
        hZeroInOriginal
    intro qbit hqNew
    have hqTargetReserve : qbit ∈ (dst.zslot i).reserve.qubits := List.mem_of_mem_take hqNew
    exact SameOutsideLayout.bit_eq_of_outside qs hOutsideTotal qbit
