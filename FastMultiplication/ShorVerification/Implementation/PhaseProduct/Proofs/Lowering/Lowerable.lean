...
inductive LowerablePhaseGate : Gate → Prop where
  ...
  | seq : ∀ (U V : Gate), LowerablePhaseGate U → LowerablePhaseGate V → LowerablePhaseGate (Gate.seq U V)
  | H : ∀ qbit, LowerablePhaseGate (Gate.H qbit)
  | X : ∀ qbit, LowerablePhaseGate (Gate.X qbit)
  | ShiftL : ∀ r n, LowerablePhaseGate (Gate.ShiftL r n)
  | ShiftR : ∀ r n, LowerablePhaseGate (Gate.ShiftR r n)
  | Negate : ∀ r, LowerablePhaseGate (Gate.Negate r)
  | AddScaled : ∀ dst src negSrc shift, LowerablePhaseGate (Gate.AddScaled dst src negSrc shift)
  | SignedPhaseProd : ∀ phi x z, LowerablePhaseGate (Gate.SignedPhaseProd phi x z)
  | CSignedPhaseProd : ∀ ctrl phi x z, LowerablePhaseGate (Gate.CSignedPhaseProd ctrl phi x z)
  | zeroExtend : ∀ r n, LowerablePhaseGate (Gate.zeroExtend r n)
  | signExtend : ∀ r n, LowerablePhaseGate (Gate.signExtend r n)
  | zeroDealloc : ∀ r n, LowerablePhaseGate (Gate.zeroDealloc r n)
  | signDealloc : ∀ r n, LowerablePhaseGate (Gate.signDealloc r n)
  | RadixReverse : ∀ r m, LowerablePhaseGate (Gate.RadixReverse r m)
...
@[simp] theorem not_QFT (r : ExtReg) : ¬ LowerablePhaseGate (Gate.QFT r) := by
  intro h; cases h
...
lemma lowerable_allocChunkGate {k : ℕ} (i : Fin k) (src dst : ExtReg) :
...
lemma lowerable_deallocChunkGate {k : ℕ} (i : Fin k) (src dst : ExtReg) :
...
lemma lowerable_compileSignedAllocationsAux {k : ℕ} (src dst : LayoutState k) :
    ∀ (n : ℕ) (hn : n ≤ k), LowerablePhaseGate (compileSignedAllocationsAux src dst n hn) := by
  intro n hn
  induction n with
  | zero =>
      exact LowerablePhaseGate.id
...
lemma lowerable_compileSignedAllocations (k : ℕ) (src dst : LayoutState k) :
...
lemma lowerable_compileSignedDeallocationsAux {k : ℕ} (src dst : LayoutState k) :
    ∀ (n : ℕ) (hn : n ≤ k), LowerablePhaseGate (compileSignedDeallocationsAux src dst n hn) := by
  intro n hn
  induction n with
  | zero =>
      exact LowerablePhaseGate.id
  | succ n ih =>
      ...
      let hn' : n ≤ k := Nat.le_trans (Nat.le_of_lt (Nat.lt_succ_self n)) hn
      let i : Fin k := ⟨n, lt_of_lt_of_le (Nat.lt_succ_self n) hn⟩
      have hz : LowerablePhaseGate (deallocChunkGate i (src.zslot i) (dst.zslot i)) :=
        lowerable_deallocChunkGate i (src.zslot i) (dst.zslot i)
      have hx : LowerablePhaseGate (deallocChunkGate i (src.xslot i) (dst.xslot i)) :=
      ...
      have hresult : LowerablePhaseGate
        ...
        LowerablePhaseGate.seq _ _ hz (LowerablePhaseGate.seq _ _ hx htail)
      simpa [hn', i] using hresult

/-- Full signed deallocation is lowerable. -/
lemma lowerable_compileSignedDeallocations (k : ℕ) (src dst : LayoutState k) :
...
lemma lowerable_compileAnnotatedOpsToSignedGateAux
    (k : ℕ) (hk : 1 < k) (phi : Angle) (phaseCoeff : Fin (q k) → ℚ)
...
lemma lowerable_compileOpsToSignedGate
    (k : ℕ) (hk : 1 < k) (phi : Angle) (x z : ExtReg)
    (layout : Gate.PhaseProductLayout x z k) (coeff : Fin (q k) → ℚ) (ops : Prog k) :
    LowerablePhaseGate (compileOpsToSignedGate k hk phi x z layout coeff ops) := by
  ...
  change
    LowerablePhaseGate
      (compileSignedAllocations k stInit stFinal ;;
        compileAnnotatedOpsToSignedGateAux k hk phi coeff stFinal (annotatePhaseTermsAux k 0 ops) ;;
        compileSignedDeallocations k stInit stFinal)
  exact LowerablePhaseGate.seq _ _
    (lowerable_compileSignedAllocations k stInit stFinal)
    (LowerablePhaseGate.seq _ _
      (lowerable_compileAnnotatedOpsToSignedGateAux k hk phi coeff stFinal
        (annotatePhaseTermsAux k 0 ops))
      (lowerable_compileSignedDeallocations k stInit stFinal))

/-- Control-phase wrapping preserves lowerability. -/
lemma lowerable_controlPhaseLeaves (ctrl : ℕ) {U : Gate} (hU : LowerablePhaseGate U) :
    LowerablePhaseGate (controlPhaseLeaves ctrl U) := by
  induction hU with
  | id => exact LowerablePhaseGate.id
  | seq U V hU hV ihU ihV => exact LowerablePhaseGate.seq _ _ ihU ihV
  | H qbit => exact LowerablePhaseGate.H qbit
  | X qbit => exact LowerablePhaseGate.X qbit
  | ShiftL r n => exact LowerablePhaseGate.ShiftL r n
  | ShiftR r n => exact LowerablePhaseGate.ShiftR r n
  | Negate r => exact LowerablePhaseGate.Negate r
  | AddScaled dst src negSrc shift => exact LowerablePhaseGate.AddScaled dst src negSrc shift
  | SignedPhaseProd phi x z => exact LowerablePhaseGate.CSignedPhaseProd ctrl phi x z
  | CSignedPhaseProd ctrl' phi x z => exact LowerablePhaseGate.CSignedPhaseProd ctrl' phi x z
  | zeroExtend r n => exact LowerablePhaseGate.zeroExtend r n
  | signExtend r n => exact LowerablePhaseGate.signExtend r n
  | zeroDealloc r n => exact LowerablePhaseGate.zeroDealloc r n
  | signDealloc r n => exact LowerablePhaseGate.signDealloc r n
  | RadixReverse r m => exact LowerablePhaseGate.RadixReverse r m
...
lemma lowerable_compileOpsToCSignedGate
    ...
    (layout : Gate.PhaseProductLayout x z k) (coeff : Fin (q k) → ℚ) (ops : Prog k) :
    LowerablePhaseGate (compileOpsToCSignedGate k hk ctrl phi x z layout coeff ops) := by
