...
theorem eval_PhaseProdUsing_ket
    ...
    (b : qs.Basis)
    (hclean : ws.Clean b) :
    qs.eval (Gate.PhaseProdUsing phi x z ws) (qs.ket b)
      =
    Complex.exp
        (((Angle.toReal phi : ℝ) : ℂ) * Complex.I *
          ((RegEncoding.toNat x b : ℂ) * (RegEncoding.toNat z b : ℂ))) •
      qs.ket b := by
  have hxFresh : ws.xExt.FreshFor 1 b := hclean.1
  have hzFresh : ws.zExt.FreshFor 1 b := hclean.2
  have hxInt : extToInt (ws.xExt.grow 1) b = (RegEncoding.toNat x b : ℤ) := by
    simpa using
      ExtReg.extToInt_grow_of_fresh (e := ws.xExt) (n := 1) (b := b) ws.xExt_canGrow hxFresh
        (by omega)
  have hzInt : extToInt (ws.zExt.grow 1) b = (RegEncoding.toNat z b : ℤ) := by
    simpa using
      ExtReg.extToInt_grow_of_fresh (e := ws.zExt) (n := 1) (b := b) ws.zExt_canGrow hzFresh
        (by omega)
  simp only [
    Gate.PhaseProdUsing,
    qs.eval_seq,
...
theorem eval_CPhaseProdUsing_ket
    ...
    (b : qs.Basis)
    (hclean : ws.Clean b) :
    qs.eval (Gate.CPhaseProdUsing ctrl phi x z ws) (qs.ket b)
      =
  ...
  have hzInt : extToInt (ws.zExt.grow 1) b = (RegEncoding.toNat z b : ℤ) := by
    simpa using
      ExtReg.extToInt_grow_of_fresh (e := ws.zExt) (n := 1) (b := b) ws.zExt_canGrow hzFresh
        (by omega)
  simp only [
    Gate.CPhaseProdUsing, qs.eval_seq, ExtensionSemantics.eval_zeroExtend,
    ExtensionSemantics.eval_zeroDealloc
  ]
