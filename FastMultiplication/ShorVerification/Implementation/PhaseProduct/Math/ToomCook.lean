...
lemma GoodInterpolationPoints.congr_matrix
  ...
  have hM : (fun i j => rowA (ptsA i) j) = (fun i j => rowB (ptsB i) j) := by
    funext i j
    exact hentry i j
...
theorem interpCoeff_correct {Point : Type u} {m : ℕ}
    ...
    (∑ i : Fin m, interpCoeff row pts B i * evalAtPoint m row polyCoeff (pts i))
      = evalAtRadix m polyCoeff B := by
  classical

  let M : Matrix (Fin m) (Fin m) ℚ := interpMatrix row pts

  let c : Matrix (Fin m) (Fin 1) ℚ := fun j _ => polyCoeff j

  have hUnit : IsUnit M.det := by
    rw [isUnit_iff_ne_zero]
    simpa [M, GoodInterpolationPoints] using hGood

  have hInv : M⁻¹ * M = 1 := by simpa using Matrix.nonsing_inv_mul M hUnit
  ...
  have hRight : evalAtRadix m polyCoeff B = (radixRow m B * c) 0 0 := by
    simp [evalAtRadix, radixRow, c, Matrix.mul_apply, mul_comm]

  have hMat : radixRow m B * M⁻¹ * (M * c) = radixRow m B * c := by
    calc
      radixRow m B * M⁻¹ * (M * c)
          = radixRow m B * (M⁻¹ * (M * c)) := by rw [Matrix.mul_assoc]
      _ = radixRow m B * ((M⁻¹ * M) * c) := by
          rw [← Matrix.mul_assoc]
          rw [Matrix.mul_assoc]
          congr 1
          rw [← Matrix.mul_assoc]

      _ = radixRow m B * (((1 : Matrix (Fin m) (Fin m) ℚ) * c)) := by rw [hInv]
      _ = radixRow m B * c := by simp

  calc
    (∑ i : Fin m, interpCoeff row pts B i * evalAtPoint m row polyCoeff (pts i))
        = (((radixRow m B * M⁻¹) * (M * c)) 0 0) := hLeft
    _ = (radixRow m B * c) 0 0 := by rw [hMat]
    _ = evalAtRadix m polyCoeff B := hRight.symm
...
def phaseFactor (phi : ℝ) (coeff : ℚ) (term : ℚ) : ℂ :=
  Complex.exp ((((phi * (coeff : ℝ)) : ℝ) : ℂ) * Complex.I * ((term : ℚ) : ℂ))
...
lemma alternatingInt_injective :
  ...
  · by_cases hj : j % 2 = 0
    · -- even/even
      ...
      have hdiv : i / 2 = j / 2 := by exact_mod_cast h
    ...
    · -- even/odd: nonnegative equals negative, contradiction
      ...
      have hj1 : j % 2 = 1 := nat_mod_two_eq_one_of_ne_zero hj

      have hj_ge : 2 ≤ j + 1 := by omega

      have hjpos_nat : 0 < (j + 1) / 2 := Nat.div_pos hj_ge (by decide : 0 < 2)

      have hnonneg : (0 : ℤ) ≤ ((i / 2 : ℕ) : ℤ) := by exact_mod_cast Nat.zero_le (i / 2)

      have hneg : -(((j + 1) / 2 : ℕ) : ℤ) < 0 := by
        have hpos : (0 : ℤ) < (((j + 1) / 2 : ℕ) : ℤ) := by exact_mod_cast hjpos_nat
        linarith
  ...
  · by_cases hj : j % 2 = 0
    · -- odd/even: negative equals nonnegative, contradiction
      ...
      have hi1 : i % 2 = 1 := nat_mod_two_eq_one_of_ne_zero hi

      have hi_ge : 2 ≤ i + 1 := by omega

      have hipos_nat : 0 < (i + 1) / 2 := Nat.div_pos hi_ge (by decide : 0 < 2)

      have hneg : -(((i + 1) / 2 : ℕ) : ℤ) < 0 := by
        have hpos : (0 : ℤ) < (((i + 1) / 2 : ℕ) : ℤ) := by exact_mod_cast hipos_nat
        linarith

      have hnonneg : (0 : ℤ) ≤ ((j / 2 : ℕ) : ℤ) := by exact_mod_cast Nat.zero_le (j / 2)
    ...
    · -- odd/odd
      ...
      have hcast : (((i + 1) / 2 : ℕ) : ℤ) = (((j + 1) / 2 : ℕ) : ℤ) := by simp_all

      have hdiv : (i + 1) / 2 = (j + 1) / 2 := by exact_mod_cast hcast

      have hi1 : i % 2 = 1 := nat_mod_two_eq_one_of_ne_zero hi

      have hj1 : j % 2 = 1 := nat_mod_two_eq_one_of_ne_zero hj

      have hisucc_mod : (i + 1) % 2 = 0 := by omega

      have hjsucc_mod : (j + 1) % 2 = 0 := by omega
...
lemma genFiniteInterpolationPoints_coord_injective
  ...
  have hij' : (alternatingInt i.1 : ℚ) = (alternatingInt j.1 : ℚ) := by
...
lemma genFiniteInterpolationPoints_good
  ...
  let ptsFin : Fin m → Point := listToFin (genFiniteInterpolationPoints m) hpts

  have hCoordGood :
      GoodInterpolationPoints (row := fun p j => pointCoordQ p ^ (j : ℕ)) (pts := ptsFin) := by
    exact GoodInterpolationPoints.of_vandermonde_distinct
      pointCoordQ ptsFin (genFiniteInterpolationPoints_coord_injective m hpts)

  apply GoodInterpolationPoints.congr_matrix
    (rowA := fun p j => pointCoordQ p ^ (j : ℕ)) (rowB := pointRow m)
    (ptsA := ptsFin) (ptsB := ptsFin)
  · intro i j
    dsimp [ptsFin]
...
lemma tailWeightedPointSum_eq_zero_of_le
  ...
  intro i hi
  have hnle : n ≤ i.1 := by exact (Finset.mem_filter.mp hi).2
  have him : i.1 < m := i.2
  exfalso
  omega
...
lemma tailWeightedPointSum_eq_cons
    ...
    tailWeightedPointSum coeff pointTerm n
      = ((coeff ⟨n, hn⟩ : ℚ) : ℂ) * ((pointTerm ⟨n, hn⟩ : ℚ) : ℂ)
...
theorem phaseScalarFromList_eq_exp_tailWeightedPointSum
    ...
    ∀ (pts : List α) (n : ℕ) (hn : n + pts.length = m),
      phaseScalarFromList phi coeff pointTerm pts n hn
        = Complex.exp ((phi : ℂ) * Complex.I * tailWeightedPointSum coeff pointTerm n) := by
  classical
  intro pts
  induction pts with
  | nil =>
      ...
      have hmn : m ≤ n := by
        simp at hn
        omega
      have htail : tailWeightedPointSum coeff pointTerm n = 0 :=
...
theorem phaseScalarFromList_eq_exp_weightedPointSum
    ...
    phaseScalarFromList phi coeff pointTerm pts 0 (by simpa using hpts)
      = Complex.exp ((phi : ℂ) * Complex.I * weightedPointSum coeff pointTerm) := by
  classical
  have h :=
    phaseScalarFromList_eq_exp_tailWeightedPointSum
      (α := α) (m := m) phi coeff pointTerm pts 0 (by simpa using hpts)

  simpa [tailWeightedPointSum, weightedPointSum] using h

lemma phaseScalarFromList_eq_exp_sum
  {Point : Type u} {k : ℕ}
  (phi : ℝ) (coeff terms : Fin (2*k-1) → ℚ)
  (pts : List Point) (hpts : pts.length = (2*k-1)) :
  ToomCookMath.phaseScalarFromList phi coeff terms pts 0 (by simpa using hpts)
    = Complex.exp (phi * Complex.I * (((∑ i : Fin (2*k-1), coeff i * terms i : ℚ) : ℂ))) := by
  simpa [ToomCookMath.weightedPointSum, mul_assoc, mul_left_comm, mul_comm] using
    (ToomCookMath.phaseScalarFromList_eq_exp_weightedPointSum
      (α := Point) (m := 2*k-1) (phi := phi)
      (coeff := coeff) (pointTerm := terms) (pts := pts) (hpts := hpts))
