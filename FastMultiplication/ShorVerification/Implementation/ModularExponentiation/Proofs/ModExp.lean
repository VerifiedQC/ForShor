...
lemma modExpTailArithmeticOK_tail
  ...
  let j : Fin (ctrl :: ctrls).length := ⟨i.1 + 1, by simp⟩
  have h' := h j
  have hexp : e + (i.1 + 1) = e + 1 + i.1 := by omega
...
private theorem ideal_preserves_algorithm1_good_ket
    ...
    (cfg : ModMulConfig η)
    (b : qs.Basis)
    (hb : GoodAlgorithm1BasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.env.scratch cfg.flag b) :
    ∃ b' : qs.Basis, qs.eval (ModMulConfig.idealGate cfg) (qs.ket b) = qs.ket b' ∧
      GoodAlgorithm1BasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.env.scratch cfg.flag b' := by
  classical

  have hbmod : GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag b := hb.1

  have hscratch_zero : RegEncoding.toNat cfg.env.scratch.active b = 0 := hb.2.1

  have hscratch_fresh : cfg.env.scratch.FreshFor 1 b := hb.2.2
  ...
  have hctrl_data : cfg.ctrl ∉ cfg.env.data.active.qubits := by
    intro hctrl
    exact cfg.layout.2.2.2.1 (List.mem_append_left _ hctrl)
  ...
  have hout_lt_cap : out < ASize cfg.env.data.active :=
    lt_of_lt_of_le hout_lt_N cfg.env.data_capacity

  have hdata_out : RegEncoding.toNat cfg.env.data.active b' = out := by
    dsimp [b']
    exact RegEncoding.toNat_writeNat_of_lt cfg.env.data.active out b hout_lt_cap

  have hdataFresh_out : cfg.env.data.FreshFor 2 b' := by
    dsimp [b']
    exact ExtReg.freshFor_write_active cfg.env.data 2 out b hbmod.2.1

  have howned : ∀ q, q ∈ cfg.env.data.ownedQubits → q ∈ cfg.env.work.ownedQubits → False := by
    have h := cfg.layout.1
    rw [ExtReg.OwnedDisjoint, List.disjoint_left] at h
    exact h

  have hwork_data : Disjoint cfg.env.work.active cfg.env.data.active := by
    rw [Shor.Disjoint, List.disjoint_left]
    intro q hqWork hqData
    exact howned q (List.mem_append_left _ hqData) (List.mem_append_left _ hqWork)

  have hworkNew_data : Disjoint (cfg.env.work.newBits 1) cfg.env.data.active := by
    rw [Shor.Disjoint, List.disjoint_left]
  ...
  have hflag_data : Disjoint (qubitReg cfg.flag) cfg.env.data.active := by
    rw [Shor.Disjoint, List.disjoint_left]
    intro q hqFlag hqData
    have hq : q = cfg.flag := by simpa [qubitReg, Reg.singleton] using hqFlag
    subst q
    exact cfg.layout.2.1 (List.mem_append_left _ hqData)

  have hwork_out : RegEncoding.toNat cfg.env.work.active b' = 0 := by
    calc
      RegEncoding.toNat cfg.env.work.active b' = RegEncoding.toNat cfg.env.work.active b := by
          dsimp [b']
          exact RegEncoding.toNat_left_write_right
            cfg.env.work.active cfg.env.data.active hwork_data b out
      _ = 0 := hbmod.2.2.1

  have hworkFresh_out : cfg.env.work.FreshFor 1 b' := by
    calc
      RegEncoding.toNat (cfg.env.work.newBits 1) b'
          = RegEncoding.toNat (cfg.env.work.newBits 1) b := by
        dsimp [b']
        exact RegEncoding.toNat_left_write_right
          (cfg.env.work.newBits 1) cfg.env.data.active hworkNew_data b out
      _ = 0 := by
        simpa [ExtReg.FreshFor, FreshZero] using hbmod.2.2.2.1

  have hflag_out : RegEncoding.toNat (qubitReg cfg.flag) b' = 0 := by
    calc
      RegEncoding.toNat (qubitReg cfg.flag) b' = RegEncoding.toNat (qubitReg cfg.flag) b := by
          dsimp [b']
          exact RegEncoding.toNat_left_write_right
            (qubitReg cfg.flag) cfg.env.data.active hflag_data b out
      _ = 0 := hbmod.2.2.2.2

  have hdataScratch : ExtReg.OwnedDisjoint cfg.env.data cfg.env.scratch := by
    have h := cfg.step4_workspace.data_scratch_disjoint
    rw [ExtReg.OwnedDisjoint, List.disjoint_left] at h ⊢
    intro q hqData hqScratch
    exact h (by simpa [Gate.ExtReg.ownedQubits_grow] using hqData) hqScratch

  have hscratch_data : Disjoint cfg.env.scratch.active cfg.env.data.active := by
    have h := hdataScratch
    rw [ExtReg.OwnedDisjoint, List.disjoint_left] at h
    rw [Shor.Disjoint, List.disjoint_left]
    intro q hqScratch hqData
    exact h (List.mem_append_left _ hqData) (List.mem_append_left _ hqScratch)

  have hscratchNew_data : Disjoint (cfg.env.scratch.newBits 1) cfg.env.data.active := by
    have h := hdataScratch
    rw [ExtReg.OwnedDisjoint, List.disjoint_left] at h
    rw [Shor.Disjoint, List.disjoint_left]
    intro q hqNew hqData
  ...
  have hscratch_zero_out : RegEncoding.toNat cfg.env.scratch.active b' = 0 := by
    calc
      RegEncoding.toNat cfg.env.scratch.active b'
          = RegEncoding.toNat cfg.env.scratch.active b := by
          dsimp [b']
          exact RegEncoding.toNat_left_write_right
            cfg.env.scratch.active cfg.env.data.active hscratch_data b out
      _ = 0 := hscratch_zero

  have hscratch_fresh_out : cfg.env.scratch.FreshFor 1 b' := by
    ...
    calc
      RegEncoding.toNat (cfg.env.scratch.newBits 1) b'
          ...
          dsimp [b']
          exact RegEncoding.toNat_left_write_right
            (cfg.env.scratch.newBits 1) cfg.env.data.active hscratchNew_data b out
      _ = 0 := hscratch_fresh

  have hgood_out : GoodModMulBasisInput qs cfg.env.N cfg.env.data cfg.env.work cfg.flag b' := by
    refine ⟨?_, hdataFresh_out, hwork_out, hworkFresh_out, hflag_out⟩
    rw [hdata_out]
    exact hout_lt_N

  have heval : qs.eval (ModMulConfig.idealGate cfg) (qs.ket b) = qs.ket b' := by
    simpa [ModMulConfig.idealGate, b', out] using
      (IdealCtrlModMulExactSemantics.eval_idealCtrlModMul_ket_exact (qs := qs)
        cfg.c cfg.env.N cfg.env.data.active cfg.ctrl b cfg.env.modulus_gt_one
        cfg.env.data_capacity cfg.coprime hctrl_data hbmod.1)

  exact ⟨b', heval, hgood_out, hscratch_zero_out, hscratch_fresh_out⟩
...
theorem ideal_preserves_valid
  ...
  let validSet : Set qs.State :=
    { ξ : qs.State |
      ∃ b : qs.Basis,
        GoodAlgorithm1BasisInput qs cfg.env.N cfg.env.data cfg.env.work
            cfg.env.scratch cfg.flag b ∧
        ξ = qs.ket b }

  change ψ ∈ Submodule.span ℂ validSet at hψ

  change qs.eval (ModMulConfig.idealGate cfg) ψ ∈ Submodule.span ℂ validSet

  refine
    Submodule.span_induction
      (s := validSet)
      (p := fun ξ _ => qs.eval (ModMulConfig.idealGate cfg) ξ ∈ Submodule.span ℂ validSet)
      ?basis
      ?zero
      ?add
      ?smul
  ...
  case basis =>
    ...
    obtain ⟨b', heval, hgood⟩ := ideal_preserves_algorithm1_good_ket qs cfg b hb
    rw [heval]
...
theorem modExpApproxSteps_valid_dist_uniform
    ...
    ∃ K : ℝ, 0 ≤ K ∧ K ≤ 2048 ∧
      ∀ (η : ℝ)
        ...
        ‖ψ‖ = 1 →
        ‖qs.eval (modExpApproxStepsValid a N data work scratch flag hworkspace hstep4 e ctrls) ψ -
  ...
  rcases modMul_approx_valid_dist_uniform (qs := qs) with ⟨K, hK_nonneg, hK_le, hmodMul⟩
  ...
  | cons ctrl ctrls ih =>
      ...
      let A : Gate := CmodMulInPlaceCore c N ctrl data work scratch flag hworkspace hstep4
      let I : Gate := Gate.idealCtrlModMul c N data.active ctrl
      let RA : Gate := modExpApproxStepsValid a N data work scratch flag hworkspace hstep4 (e + 1) ctrls
      let RI : Gate := modExpIdealSteps qs a N data.active (e + 1) ctrls

      have hApprox :
          modExpApproxStepsValid a N data work scratch flag hworkspace hstep4 e (ctrl :: ctrls) = A ;; RA := by
        simp [modExpApproxStepsValid, A, RA, c]

      have hIdeal : modExpIdealSteps qs a N data.active e (ctrl :: ctrls) = I ;; RI := by
      ...
      have hHeadUnit : ModMulConfig.ValidUnitState qs headCfg ψ := ⟨hHeadValid, hNorm⟩

      -- Apply the one-step modular-multiplication estimate.
      have hHead : ‖ψA0 - ψI0‖ ≤ stepErr K η := by
        simpa [ψA0, ψI0, A, I, ModMulConfig.approxGate, ModMulConfig.idealGate, headCfg, headEnv]
          using (hmodMul η headCfg ψ hHeadUnit)
      ...
      have hψI0Valid : ψI0 ∈ ValidAlgorithm1State qs N data work scratch flag := by
        simpa [ModMulConfig.ValidState, headCfg, headEnv] using hψI0ValidCfg

      -- Apply the induction hypothesis to the ideal head output.
      have hTail : ‖qs.eval RA ψI0 - qs.eval RI ψI0‖ ≤ (ctrls.length : ℝ) * stepErr K η := by
        have h := ih (e := e + 1) (ψ := ψI0) hTailLayout hTailArithmetic hψI0Valid hψI0Norm
        simpa [RA, RI] using h

      have hIso : ‖qs.eval RA ψA0 - qs.eval RA ψI0‖ = ‖ψA0 - ψI0‖ := by
      ...
      have hTriangle : ‖qs.eval RA ψA0 - qs.eval RI ψI0‖
            ≤ ‖qs.eval RA ψA0 - qs.eval RA ψI0‖ + ‖qs.eval RA ψI0 - qs.eval RI ψI0‖ := by
        rw [show qs.eval RA ψA0 - qs.eval RI ψI0 =
          (qs.eval RA ψA0 - qs.eval RA ψI0) + (qs.eval RA ψI0 - qs.eval RI ψI0) by abel]
        exact norm_add_le _ _

      have hMain : ‖qs.eval RA ψA0 - qs.eval RI ψI0‖
            ≤ ((ctrls.length + 1 : ℕ) : ℝ) * stepErr K η := by
        calc
          ‖qs.eval RA ψA0 - qs.eval RI ψI0‖ ≤ ‖qs.eval RA ψA0 - qs.eval RA ψI0‖ +
...
theorem modExpApprox_valid_dist_uniform
    ...
    ∃ K : ℝ, 0 ≤ K ∧ K ≤ 2048 ∧
      ∀ (η : ℝ) (cfg : ModExpConfig η) (ψ : qs.State),
        ...
        ‖qs.eval (ModExpConfig.approxGate cfg) ψ - qs.eval (ModExpConfig.idealGate qs cfg) ψ‖
          ≤ (tbits cfg.x : ℝ) * stepErr K η := by
  rcases modExpApproxSteps_valid_dist_uniform (qs := qs) with ⟨K, hK_nonneg, hK_le, hSteps⟩
  ...
  have hTailLayout : ModExpTailLayout cfg.env.data cfg.env.work cfg.flag cfg.x.qubits := by
    intro i
  ...
  have hTailArithmetic : ModExpTailArithmeticOK cfg.a cfg.env.N 0 cfg.x.qubits := by
    intro i
    let j : Fin (regSize cfg.x) :=
      ⟨i.1, by simp [regSize, Reg.width]⟩
