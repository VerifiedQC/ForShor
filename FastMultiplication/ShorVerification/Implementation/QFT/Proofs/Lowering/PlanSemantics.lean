...
theorem eval_QFT_size0_ket
    {qs : QSemantics} [RegEncoding qs.Basis] [GateSemanticsCore qs] [QFTSemantics qs]
    (r : ExtReg) (b : qs.Basis) (hsize : r.width = 0) :
  ...
  have hwrite : RegEncoding.writeNat r.active 0 b = b := by
    rw [← hread]
...
theorem eval_QFT_size0
    {qs : QSemantics} [RegEncoding qs.Basis] [GateSemanticsCore qs] [QFTSemantics qs]
    (r : ExtReg) (ψ : qs.State) (hsize : r.width = 0) :
    qs.eval (Gate.QFT r) ψ = qs.eval Gate.id ψ := by
  have h : ∀ φ : qs.State, qs.eval (Gate.QFT r) φ = φ := by
    intro φ

    apply qs.state_induction (P := fun φ => qs.eval (Gate.QFT r) φ = φ)
...
theorem eval_QFT_size1_ket
    {qs : QSemantics} [RegEncoding qs.Basis] [GateSemanticsCore qs] [QFTSemantics qs]
    [HadamardSemantics qs] (r : ExtReg) (b : qs.Basis) (hsize : r.width = 1) :
  ...
  let hpos : 0 < regSize r.active := by omega

  let q : ℕ := r.active.lowQubit hpos

  change qs.eval (Gate.QFT r) (qs.ket b) = qs.eval (Gate.H q) (qs.ket b)

  have hactive : r.active = qubitReg q := by
    simpa [q] using Reg.eq_qubitReg_lowQubit r.active hactive_size

  -- The value stored in the one-bit register is either 0 or 1.
  have hxlt : ExtReg.toNat r b < 2 := by
    have h := ExtReg.toNat_lt r b
    simpa [hsize] using h

  -- Logical bit 0 of the active register is precisely physical qubit q.
  have hbit : RegEncoding.bit q b = Nat.testBit (ExtReg.toNat r b) 0 := by
    have h := RegEncoding.bit_eq_testBit_toNat
      (qubitReg q) b (⟨0, by simp⟩ : Fin (regSize (qubitReg q)))

    simpa [ExtReg.toNat, hactive, qubitReg, Reg.singleton, Reg.get, regSize, Reg.width] using h

  -- Hence the numeric value of this one-bit register is exactly its bit.
  have hx : ExtReg.toNat r b = if RegEncoding.bit q b then 1 else 0 := by
    have hx_cases : ExtReg.toNat r b = 0 ∨ ExtReg.toNat r b = 1 := by omega

    rcases hx_cases with hx0 | hx1
    · have hb : RegEncoding.bit q b = false := by simpa [hx0] using hbit
      simp [hx0, hb]

    · have hb : RegEncoding.bit q b = true := by simpa [hx1] using hbit
      simp [hx1, hb]

  -- y = 0 contributes phase 1.
  have hphase0 : qftPhase 2 (ExtReg.toNat r b) 0 = 1 := by
    simp [qftPhase, ωPow]

  -- y = 1 contributes +1 or -1 according to the input bit.
  have hphase1 : qftPhase 2 (ExtReg.toNat r b) 1 =
...
theorem eval_QFT_size1
    {qs : QSemantics} [RegEncoding qs.Basis] [GateSemanticsCore qs] [QFTSemantics qs]
    [HadamardSemantics qs] (r : ExtReg) (ψ : qs.State) (hsize : r.width = 1) :
    qs.eval (Gate.QFT r) ψ =
      qs.eval (Gate.H (r.active.lowQubit (by
        simp [ExtReg.width] at hsize
        omega))) ψ := by
  have h : ∀ φ : qs.State, qs.eval (Gate.QFT r) φ =
        qs.eval (Gate.H (r.active.lowQubit (by
          simp [ExtReg.width] at hsize
          omega))) φ := by
    intro φ

    apply qs.state_induction (P := fun φ =>
    ...
    · intro φ χ hφ hχ
      ...
      rw [GateSemanticsCore.eval_add, GateSemanticsCore.eval_add, hφ, hχ]

    · intro a φ hφ
      unfold QSemantics.eval at *
      rw [GateSemanticsCore.eval_smul, GateSemanticsCore.eval_smul, hφ]
...
theorem evalL_lowerQFTPlan
  ...
  | empty r hsize =>
      have hQFT := QFTSemantics.eval_QFT_size0 (qs := qs) (r := ExtReg.ofReg r) (ψ := ψ) hsize

      calc
        LowerGateClass.evalL (qs := qs) (lowerQFTPlan (QFTLoweringPlan.empty r hsize)) ψ
            =
          ψ := by
            simp only [lowerQFTPlan, LowerGateClass.evalL_id]
  ...
  | singleton r hsize =>
      have hQFT := QFTSemantics.eval_QFT_size1 (qs := qs) (r := ExtReg.ofReg r) (ψ := ψ) hsize

      calc
        LowerGateClass.evalL (qs := qs) (lowerQFTPlan (QFTLoweringPlan.singleton r hsize)) ψ
            =
          qs.eval (Gate.H (r.lowQubit (by omega))) ψ := by
              simp only [lowerQFTPlan, LowerGateClass.evalL_H]

        _ =
          qs.eval (Gate.QFT (ExtReg.ofReg r)) ψ := by
            exact hQFT.symm

  | split r hsize ws phaseInitSize phasePlan rightPlan leftPlan ihRight ihLeft =>
      ...
      change
        ...
        QFTLoweringReady qs rightPlan ψ
          ∧
        PhaseLoweringReady qs phasePlan
          (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) ψ)
          ∧
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) ψ))
        at hready

      rcases hready with ⟨hclean, hreadyRight, hreadyPhase, hreadyLeft⟩
      ...
      have hLeft :
          LowerGateClass.evalL (qs := qs) (lowerQFTPlan leftPlan)
              (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
                (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) ψ))
            =
          qs.eval (Gate.QFT (ExtReg.ofReg (leftReg r)))
              (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
                (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) ψ)) := by
        exact ihLeft _ hreadyLeft

      have hSplit :
          qs.eval (Gate.QFT (ExtReg.ofReg r)) ψ
            =
          qs.eval
            (Gate.QFT (ExtReg.ofReg (rightReg r)) ;;
              Gate.PhaseProdUsing (qftPhi (regSize r)) (leftReg r) (rightReg r) ws ;;
              Gate.QFT (ExtReg.ofReg (leftReg r)) ;;
              Gate.RadixReverse r (splitM r))
            ψ := by
        simpa [splitM, leftQFTReg, rightQFTReg, ExtReg.width] using
          eval_QFT_split (qs := qs) (ExtReg.ofReg r) ws ψ hclean hsize

      simp only [lowerQFTPlan, LowerGateClass.evalL_seq]
...
lemma evalL_lowerQFTPlan_add
    ...
    LowerGateClass.evalL (qs := qs) (lowerQFTPlan plan) ψ
      +
    LowerGateClass.evalL (qs := qs) (lowerQFTPlan plan) φ := by
  ...
  | split r hsize ws phaseInitSize phasePlan rightPlan leftPlan ihRight ihLeft =>
      ...
      change
        ...
        QFTLoweringReady qs rightPlan φ
          ∧
        PhaseLoweringReady qs phasePlan
          (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) φ)
          ∧
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) φ))
        at hφ
      rcases hψ with ⟨hcleanψ, hrightψ, hphaseψ, hleftψ⟩
      rcases hφ with ⟨hcleanφ, hrightφ, hphaseφ, hleftφ⟩
      change
        ...
        QFTLoweringReady qs rightPlan (ψ + φ)
          ∧
        PhaseLoweringReady qs phasePlan
          (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) (ψ + φ))
          ∧
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) (ψ + φ)))
      rw [evalL_lowerQFTPlan_add, evalL_lowerGateRec_add]
      exact ⟨CleanClosure.add hcleanψ hcleanφ, ihRight hrightψ hrightφ,
        PhaseLoweringReady.add qs phasePlan hphaseψ hphaseφ, ihLeft hleftψ hleftφ⟩
      ...
      change
        ...
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) ψ))
        at hψ
      rcases hψ with ⟨hclean, hright, hphase, hleft⟩
      change
        ...
        QFTLoweringReady qs rightPlan (a • ψ)
          ∧
        PhaseLoweringReady qs phasePlan
          (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) (a • ψ))
          ∧
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) (a • ψ)))
      rw [evalL_lowerQFTPlan_smul, evalL_lowerGateRec_smul]
      exact ⟨CleanClosure.smul a hclean, ihRight hright,
        PhaseLoweringReady.smul qs phasePlan a hphase, ihLeft hleft⟩
  ...
  | split r hsize ws phaseInitSize phasePlan rightPlan leftPlan ihRight ihLeft =>
      change
        ...
        QFTLoweringReady qs rightPlan 0
          ∧
        PhaseLoweringReady qs phasePlan
          (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) 0)
          ∧
        QFTLoweringReady qs leftPlan
          (LowerGateClass.evalL (qs := qs) (lowerGateRec phasePlan)
            (LowerGateClass.evalL (qs := qs) (lowerQFTPlan rightPlan) 0))
      rw [evalL_lowerQFTPlan_zero, evalL_lowerGateRec_zero]
      exact ⟨CleanClosure.zero, ihRight, PhaseLoweringReady.zero qs phasePlan, ihLeft⟩
