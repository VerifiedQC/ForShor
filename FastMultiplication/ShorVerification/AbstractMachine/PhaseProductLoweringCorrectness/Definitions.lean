...
/-! =========================================================
    ...
    `LowerGateClass` is the semantic bridge for lowered gates: each low-level
    constructor is required to evaluate like its corresponding high-level gate,
    and adjoints can be transported across an established gate equivalence.
========================================================= -/

/-- Semantic interface for interpreting low-level gates in a quantum semantics. -/
class LowerGateClass
    (qs : QSemantics) [RegEncoding qs.Basis] : Type where
  ...
  evalL_H : ∀ (qbit : ℕ) (ψ : qs.State), evalL (LowGate.H qbit) ψ = qs.eval (Gate.H qbit) ψ

  evalL_X : ∀ (qbit : ℕ) (ψ : qs.State), evalL (LowGate.X qbit) ψ = qs.eval (Gate.X qbit) ψ

  evalL_Prim : ∀ (tag : String) (args : List ℕ) (ψ : qs.State),
    evalL (LowGate.Prim tag args) ψ = qs.eval (Gate.Prim tag args) ψ

  evalL_shiftL : ∀ r n ψ, evalL (LowGate.ShiftL r n) ψ = qs.eval (Gate.ShiftL r n) ψ

  evalL_shiftR : ∀ r n ψ, evalL (LowGate.ShiftR r n) ψ = qs.eval (Gate.ShiftR r n) ψ

  evalL_negate : ∀ r ψ, evalL (LowGate.Negate r) ψ = qs.eval (Gate.Negate r) ψ

  evalL_addScaled :
    ∀ dst src negSrc shift ψ,
      evalL (LowGate.AddScaled dst src negSrc shift) ψ
        =
      qs.eval (Gate.AddScaled dst src negSrc shift) ψ

  evalL_naive_signedPhaseProd :
    ∀ phi x z ψ,
      evalL (LowGate.Naive_SignedPhaseProd phi x z) ψ
        =
      qs.eval (Gate.SignedPhaseProd phi x z) ψ

  evalL_naive_csignedPhaseProd :
    ∀ ctrl phi x z ψ,
      evalL (LowGate.Naive_CSignedPhaseProd ctrl phi x z) ψ
        =
      qs.eval (Gate.CSignedPhaseProd ctrl phi x z) ψ

  evalL_zeroExtend : ∀ r n ψ, evalL (LowGate.zeroExtend r n) ψ = qs.eval (Gate.zeroExtend r n) ψ

  evalL_signExtend : ∀ r n ψ, evalL (LowGate.signExtend r n) ψ = qs.eval (Gate.signExtend r n) ψ

  evalL_zeroDealloc : ∀ r n ψ, evalL (LowGate.zeroDealloc r n) ψ = qs.eval (Gate.zeroDealloc r n) ψ

  evalL_signDealloc : ∀ r n ψ, evalL (LowGate.signDealloc r n) ψ = qs.eval (Gate.signDealloc r n) ψ

  evalL_radixReverse : ∀ r m ψ, evalL (LowGate.RadixReverse r m) ψ = qs.eval (Gate.RadixReverse r m) ψ
