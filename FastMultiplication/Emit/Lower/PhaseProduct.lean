        ...
        if hws : SignedRecursiveWorkspaceOK ops x z then
          ...
          if annotated then
            ...
            | .ok doc =>
                ...
                let instantiateEqReal := Reflect.phaseProductAgrees hk ops doc x z phi hws
            ...
            return .ok (emitLowGateDoc (lowerSignedPhaseProdWithWorkspace k hk phi x z ops hws) metaJ)
        else
          ...
          if annotated then
            ...
            | .ok doc =>
                ...
                let instantiateEqReal :=
                  Reflect.cPhaseProductAgrees hk ops doc ctrl x z phi hws
                let (checksJ, allOk) := checksJson check1 instantiateEqReal
                let metaJ :=
                  Json.mkObj [
