...
/-! ## The `hrec` branch: connecting the deallocation leaves

`r2_2_ppThen`'s remaining structure (`.seq [alloc, .seq [body, dealloc]]`,
confirmed by direct inspection, same method as the rest of this file) and
the 5 deallocation leaves, mirroring the allocation pattern above against
`lowerGateRec_planCompileSignedDeallocations_2`/`lowerGateRec_planDeallocChunkGate`. -/

def r2_2_ppRest : Node := match r2_2_ppThen with | .seq [_, r] => r | n => n
def r2_2_ppBodyNode : Node := match r2_2_ppRest with | .seq [b, _] => b | n => n
def r2_2_ppDealloc : Node := match r2_2_ppRest with | .seq [_, d] => d | n => n

theorem r2_2_ppThen_eq_seq : r2_2_ppThen = .seq [r2_2_ppAlloc, r2_2_ppRest] := by rfl
theorem r2_2_ppRest_eq_seq : r2_2_ppRest = .seq [r2_2_ppBodyNode, r2_2_ppDealloc] := by rfl

set_option maxHeartbeats 1000000 in
theorem r2_2_ppDealloc_eq : r2_2_ppDealloc =
    .seq [.cond (.eq r2_2_growDeltaZ1W (.lit 0)) (.op "id" [] [] none [])
            (.op "signDealloc" [r2_2_ext1z] [r2_2_growDeltaZ1W] none []),
      .seq [.cond (.eq r2_2_growDeltaX1W (.lit 0)) (.op "id" [] [] none [])
              (.op "signDealloc" [r2_2_ext1x] [r2_2_growDeltaX1W] none []),
        .seq [.cond (.eq r2_2_growDeltaZ0W (.lit 0)) (.op "id" [] [] none [])
                (.op "zeroDealloc" [r2_2_ext0z] [r2_2_growDeltaZ0W] none []),
          .seq [.cond (.eq r2_2_growDeltaX0W (.lit 0)) (.op "id" [] [] none [])
                  (.op "zeroDealloc" [r2_2_ext0x] [r2_2_growDeltaX0W] none []),
                .op "id" [] [] none []]]]] := by
  rfl

/-- `evalNode` of the extracted chunk-0 `x`-side deallocation `.cond` node
equals the real `lowerGateRec (planDeallocChunkGate ...)` at chunk 0's `x`
child/grown-child — same guard-matching argument as `evalNode_pp_cond0x`. -/
theorem evalNode_pp_dealloc0x (x z : ExtReg) (phi : Angle) (d : Doc) (fuel : ℕ)
    (hrec : nextSignedWidth x z r2_2_ops < phaseInputSize x z)
    (hworkspace : SignedRecursiveWorkspaceOK r2_2_ops x z) :
    evalNode d fuel (ppEnv x z phi)
        (.cond (.eq r2_2_growDeltaX0W (.lit 0)) (.op "id" [] [] none [])
          (.op "zeroDealloc" [r2_2_ext0x] [r2_2_growDeltaX0W] none [])) =
      .ok (lowerGateRec (planDeallocChunkGate (k := r2_2_k) (hk := r2_2_hk)
        (pts := genInterpolationPoints r2_2_k) (hpts := generatedInterpolationPoints_length r2_2_k)
        (ops := r2_2_ops) (nextSignedWidth x z r2_2_ops) (0 : Fin 2)
        ((canonicalSignedStep r2_2_hk r2_2_ops x z hrec hworkspace).layout.xSplit.child 0)
        (growExtRegTo ((canonicalSignedStep r2_2_hk r2_2_ops x z hrec hworkspace).layout.xSplit.child 0)
          (nextSignedWidth x z r2_2_ops)))) := by
  rw [lowerGateRec_planDeallocChunkGate]
  have hdelta := extraDelta_grow0x x z hrec hworkspace
  simp only [evalNode, evalProp, evalW_pp_growDeltaX0, hdelta, evalW, Except.instMonad, Monad.toBind,
    Except.bind, Except.pure]
  by_cases h0 : nextSignedWidth x z r2_2_ops - phaseLimbWidth x z r2_2_k = 0
...
/-- Same as `evalNode_pp_dealloc0x`, `z`'s chunk 0. -/
theorem evalNode_pp_dealloc0z (x z : ExtReg) (phi : Angle) (d : Doc) (fuel : ℕ)
    (hrec : nextSignedWidth x z r2_2_ops < phaseInputSize x z)
    (hworkspace : SignedRecursiveWorkspaceOK r2_2_ops x z) :
    evalNode d fuel (ppEnv x z phi)
        (.cond (.eq r2_2_growDeltaZ0W (.lit 0)) (.op "id" [] [] none [])
...
/-- Same as `evalNode_pp_dealloc0x`, `x`'s chunk 1 (`signDealloc`, since chunk
1 is the top chunk). -/
theorem evalNode_pp_dealloc1x (x z : ExtReg) (phi : Angle) (d : Doc) (fuel : ℕ)
    (hrec : nextSignedWidth x z r2_2_ops < phaseInputSize x z)
    (hworkspace : SignedRecursiveWorkspaceOK r2_2_ops x z) :
    evalNode d fuel (ppEnv x z phi)
        (.cond (.eq r2_2_growDeltaX1W (.lit 0)) (.op "id" [] [] none [])
...
/-- Same as `evalNode_pp_dealloc1x`, `z`'s chunk 1. -/
theorem evalNode_pp_dealloc1z (x z : ExtReg) (phi : Angle) (d : Doc) (fuel : ℕ)
    (hrec : nextSignedWidth x z r2_2_ops < phaseInputSize x z)
    (hworkspace : SignedRecursiveWorkspaceOK r2_2_ops x z) :
    evalNode d fuel (ppEnv x z phi)
        (.cond (.eq r2_2_growDeltaZ1W (.lit 0)) (.op "id" [] [] none [])
